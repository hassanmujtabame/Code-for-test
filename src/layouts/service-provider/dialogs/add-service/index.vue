<template>
  <d-dialog-large :xl="true" :group="group" :closeDialog="closeDialog" :openDialog="openDialog" :loading="loading">
    <ValidationObserver v-if="showDialog" ref="form">
      <div class="row add-portfolio m-3 p-0">
        <div class="col-12 col-lg-4 justify-content-center mx-auto">
          <div class="col-md-12">
            <label for="imginput" class="form-label file-label first w-100">
              ...
              <div class="add-img-selected">
                <img class="w-100 h-100 image-selected-dialog" :style="{ opacity: showImage ? '1' : '0' }"
                  :src="showImage ?? 'none'" :id="idImage" />
              </div>
            </label>
          ...
          <div class="col-md-12">
            <div class="row" style="max-height: 295px;overflow: auto;">
              <!-- show galleries-->
              <div class="col-md-6 my-1" v-for="(g, i) in gallariesUrl" :key="i">
          ...
          <div class="col-md-12">
            ...
            <div class="text">
              ...
              <ValidationProvider :name="$t('File')" vid="file" rules="required|image" v-slot="{ validate, errors }">
                <input name="uploadDocument" @change="uploadGallary($event, validate) || validate($event)" type="file"
                  id="choose-file" accept=".jpg, .jpeg, .png" style="display: none;" />
                <d-error-input :errors="errors" v-if="errors.length > 0" />
              </ValidationProvider>
            </div>
          </div>
        ...
        <div class="col-12 col-lg-8">
          ...
          <!--details-->
          <div class="my-3">
            <ValidationProvider :name="$t('service-description')" vid="desc" rules="required" v-slot="{ errors }">
          ...
          <div class="mb-3">
            <ValidationProvider :name="$t('keywords')" vid="keywords" rules="required" v-slot="{ errors }">
              ...
              <d-multi-select-tag v-model="itemForm.keywords"></d-multi-select-tag>
              <d-error-input :errors="errors" v-if="errors.length > 0" />
            </ValidationProvider>
          </div>
        </div>
        <div v-if="false" class="mb-3 row">
          ...
          <div class="col-md-12">
            <div style="height:50px;width:50px" class="position-relative">
              ...
              <input @change="uploadGallary($event)" multiple="multiple" class="form-control hidden-file-input"
                type="file" />
            </div>
          </div>
          <!-- gallary show -->
          <div class="col-12">
            <div class="list-add-gallary mt-1">
              <div style="height:100px;width:100px" class="bx-img-gallary" v-for="(g, i) in gallariesUrl" :key="i">
    ...
    <template v-slot:actions>
      <button @click="save" type="button" class="btn btn-main" style="width: 25% !important;">{{ $t('publish-service')
      }}</button>
    </template>
  </d-dialog-large>
</template>
