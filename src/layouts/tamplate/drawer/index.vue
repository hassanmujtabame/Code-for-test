<template>
  <div
    class="offcanvas drawer-side"
    :class="{
      'offcanvas-end': $i18n.locale == 'ar',
      'offcanvas-start': $i18n.locale == 'en',
      show: drawer,
    }"
    tabindex="-1"
    id="offcanvas-profile"
      ...
      <div class="d-flex flex-column service-provider-navbar">
        <div class="box mx-2 mt-3">
          ...
          <router-link
            custom
            v-slot="{ navigate }"
            :to="getRouteLocale(dashboard)"
            class="mx-2 m-c"
          >
            <button
              @click="clickLink(navigate, $event)"
              role="link"
              class="btn-drawer"
            >
              {{ $t("Settings") }}
        ...
        <div style="color: #2cb7b3" class="git-hash">
          {{ gitTag }} - {{ gitHash }}
        </div>
      </div>
    </div>
