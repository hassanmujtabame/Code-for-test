<template>
  <div class="container mt-5 mb-5" >
    <div class="d-flex justify-content-between align-items-center container">
      ...
      <div>
        <router-link class="router-link d-flex align-items-center" custom
          :to="getRouteLocale('service-provider-proposals')" v-slot="{ navigate }">
      ...
      <div v-for="(item, i) in items" :key="i">
        <div v-if="item.state != 'offline'">
          <showProposal :title="item.title" :description="item.description" :category="item.field_name"
            ...
            :datePublish="item.created_at" :image="item.image" :executionTime="item.execution_period"
            :resetDays="item.rest_days" :department="item.category_name" :state="item.state" :id="item.id"
    ...
    async loadList() {
      ...
      try {
        ...
        if (data.success) {
          ...
          console.log("test_state_data", data.data)
        }
      } catch (error) {
