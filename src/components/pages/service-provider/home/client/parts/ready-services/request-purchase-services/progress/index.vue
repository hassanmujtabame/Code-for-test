    ...
    <div class="row justify-content-between">
        ...
        <div class="col-md-6 d-flex gap-2 justify-content-end">
            <div>
                ...
                <div v-if="itemPage.status != 'finished' && readyService.user_info.id == user.id" class="dropdown">
                    <button style="height: 40px; background-color:#FFBC00 ;"
                        class="btn border-0 text-white dropdown-toggle " type="button" data-bs-toggle="dropdown"
                        aria-expanded="false">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M13.6552 22.75H6.2852C3.5452 22.75 1.3252 20.52 1.3252 17.79V10.47C1.3252 7.73001 3.5552 5.51001 6.2852 5.51001H13.6552C16.3952 5.51001 18.6152 7.74001 18.6152 10.47V17.79C18.6152 20.52 16.3852 22.75 13.6552 22.75ZM6.2852 7.01001C4.3752 7.01001 2.8252 8.56001 2.8252 10.47V17.79C2.8252 19.7 4.3752 21.25 6.2852 21.25H13.6552C15.5652 21.25 17.1152 19.7 17.1152 17.79V10.47C17.1152 8.56001 15.5652 7.01001 13.6552 7.01001H6.2852V7.01001Z"
                                fill="white" />
                            <path
                                d="M5.5752 4.75C5.1652 4.75 4.8252 4.41 4.8252 4V2.25C4.8252 1.84 5.1652 1.5 5.5752 1.5C5.9852 1.5 6.3252 1.84 6.3252 2.25V4C6.3252 4.41 5.9852 4.75 5.5752 4.75Z"
                                fill="white" />
                            <path
                                d="M9.5752 4.75C9.1652 4.75 8.8252 4.41 8.8252 4V2.25C8.8252 1.84 9.1652 1.5 9.5752 1.5C9.9852 1.5 10.3252 1.84 10.3252 2.25V4C10.3252 4.41 9.9852 4.75 9.5752 4.75Z"
                                fill="white" />
                            <path
                                d="M13.5752 4.75C13.1652 4.75 12.8252 4.41 12.8252 4V2.25C12.8252 1.84 13.1652 1.5 13.5752 1.5C13.9852 1.5 14.3252 1.84 14.3252 2.25V4C14.3252 4.41 13.9852 4.75 13.5752 4.75Z"
                                fill="white" />
                            <path
                                d="M17.8652 18.12C17.4552 18.12 17.1152 17.78 17.1152 17.37V8.94995C17.1152 8.53995 17.4552 8.19995 17.8652 8.19995C20.6052 8.19995 22.8252 10.43 22.8252 13.16C22.8252 15.89 20.5952 18.12 17.8652 18.12ZM18.6152 9.77995V16.54C20.1652 16.2 21.3252 14.81 21.3252 13.16C21.3252 11.51 20.1652 10.12 18.6152 9.77995Z"
                                fill="white" />
                            <path
                                d="M17.5852 12.75H2.0752C1.6652 12.75 1.3252 12.41 1.3252 12C1.3252 11.59 1.6652 11.25 2.0752 11.25H17.5852C17.9952 11.25 18.3352 11.59 18.3352 12C18.3352 12.41 17.9952 12.75 17.5852 12.75Z"
                                fill="white" />
                        </svg>
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li><a class="dropdown-item clickable" @click="openChangeDateDeliveryDialog">تعديل موعد التسليم</a>
                        </li>
                        <li><a class="dropdown-item clickable" @click="openConfirmFinishDialog"> حذف و أنهاء المشروع</a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div>
                <button v-if="itemPage.status != 'finished' && itemPage.user_info.id == user.id"
                    @click="confirmDeliveredProductMsg" style="height: 40px;" class="btn-main px-3 w-100 border-0 rounded-2"
                    role="button">
        ...
        <div class="row redy-services ">
            <div class="col-md-6 mt-5 ">
                ...
                <!-- section details-->
                <SectionDetails :item-page="itemPage" :readyService="readyService" />
                <!-- section-conversation-->
                <SectionConversation :item-page="itemPage" :readyService="readyService" />
            </div>
            ...
            <div class="col-md-6 mt-5 ">
                ...
                <div v-if="isOwner" class="box border rounded-3 p-4 mt-3 text-center">
                    <router-link :to="getRouteLocale('service-provider-purchase-requests-one-service', { id: readyService.id })"
                        class="fs-3 ">
...
<script>
import requestPurchasesAPIs from '@/services/api/service-provider/provider/request-purchases.js'
...
export default {
    ...
    methods: {
        ...
        async confirmDeliveredProduct() {
            try {
                let { data } = await requestPurchasesAPIs.confirmDelivery(this.itemPage.user_offer.id)
        ...
        openConfirmFinishDialog(evt) {
            ...
            let dataEvent = {
                ...
                description: '',
                btns: [
                    { title: '', action: () => requestPurchasesAPIs.finishedRequestPurchase(this.itemPage.id), class: 'btn btn-custmer' }
                ]
            }
            this.showConfirmMsg(dataEvent)
