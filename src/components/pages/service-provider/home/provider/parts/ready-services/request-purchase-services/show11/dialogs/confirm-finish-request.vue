...
<svg width="160" height="160" viewBox="0 0 160 160" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 <p class="mt-1">
  ...
  {{ $t('confirm_delete_finish_request_message') }}

 </p>
 <p class="t-c m-0">عند تأكيد سنرسل الى العميل طلب انهاء المشروع فإذا وافق العميل على طلبك سيتم انهاء مشروعك
 </p>
</div>
    </template>
...
export default {
 props:{
    group:{
        type:String,
        default:'confirm-finish-request'
    }
 },
 data:()=>({
    itemDialog:{id:null},
 ...
 methods:{
    async delBlog(){
        try {
            let {data} = await readyServiceAPI.finisheRequestsForPurchaseService(this.itemDialog.id)
            if(data.success){
