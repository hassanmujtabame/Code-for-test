<template>
  <div class="box border rounded-top" v-bind="$attrs">
    <div class="position-relative">
      ...
      <img class="w-100 rounded-top" :src="img" :alt="title" height="167" />
      <button v-if="isShared" class="btn-shared-exibition">
        <tickCircleIcon />{{ $t("shared") }}
      </button>
    </div>
    <div class="p-2 text-start">
      <h6 style="color: black" class="exhibition-card-title mt-2">
        {{ title }}
      </h6>
      <p
        style="font-size: 14px"
        class="t-c text-two-lines exhibition-card-desc exhibition-card-text"
        v-html="description"
      ></p>
      <div class="d-flex justify-content-between flex-wrap">
        <p style="font-size: 14px" class="exhibition-card-text">
          <d-empty-wallet-icon :size="18" color="currentColor" />
        ...
        <p style="font-size: 14px" class="exhibition-card-text">
          <d-localisation-icon :size="18" color="currentColor" />
          {{ place }}
        </p>
        <p style="font-size: 14px" class="exhibition-card-text">
          <d-school-wallet-icon :size="18" color="currentColor" />
