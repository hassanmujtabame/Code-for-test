<template>
   <div class="box border rounded-3">
                    <div class="p-2">

                        <p class="text-end t-c">
                            تاريخ النشر 
                            <span>
                                : {{datePublish}} 
                            </span>
                        </p>
                        <h5>
                        {{title}}
                        </h5>
                        <p>{{description}}</p>
                    </div>
                    <div style="background-color: #414042d4;" class="col-md-12  dashbord rounded-bottom">
               
    
          
                        <div class=" row justify-content-between p-1 ">
                            <div class="d-flex col-md-4">
                                <div class="t-c">

                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M15.8101 20.1795C15.5501 20.1795 15.2801 20.1695 14.9901 20.1395C14.4701 20.0995 13.8801 19.9995 13.2701 19.8495L11.5901 19.4495C6.98007 18.3595 5.47007 15.9195 6.55007 11.3195L7.53007 7.12953C7.75007 6.17953 8.01007 5.40953 8.33007 4.76953C10.0501 1.21953 13.3401 1.53953 15.6801 2.08953L17.3501 2.47953C19.6901 3.02953 21.1701 3.89953 22.0001 5.22953C22.8201 6.55953 22.9501 8.26953 22.4001 10.6095L21.4201 14.7895C20.5601 18.4495 18.7701 20.1795 15.8101 20.1795ZM13.1201 3.24953C11.4501 3.24953 10.3901 3.93953 9.68007 5.41953C9.42007 5.95953 9.19007 6.62953 8.99007 7.46953L8.01007 11.6595C7.12007 15.4395 8.15007 17.0895 11.9301 17.9895L13.6101 18.3895C14.1501 18.5195 14.6601 18.5995 15.1201 18.6395C17.8401 18.9095 19.1901 17.7195 19.9501 14.4495L20.9301 10.2695C21.3801 8.33953 21.3201 6.98953 20.7201 6.01953C20.1201 5.04953 18.9401 4.38953 17.0001 3.93953L15.3301 3.54953C14.5001 3.34953 13.7601 3.24953 13.1201 3.24953Z" fill="white"/>
                                        <path d="M8.32907 22.2497C5.75908 22.2497 4.11907 20.7097 3.06907 17.4597L1.78907 13.5097C0.369075 9.10968 1.63907 6.62968 6.01907 5.20968L7.59907 4.69968C8.11907 4.53968 8.50907 4.42968 8.85907 4.36968C9.14907 4.30968 9.42908 4.41968 9.59908 4.64968C9.76908 4.87968 9.79908 5.17968 9.67908 5.43968C9.41908 5.96968 9.18907 6.63968 8.99907 7.47968L8.01908 11.6697C7.12908 15.4497 8.15907 17.0997 11.9391 17.9997L13.6191 18.3997C14.1591 18.5297 14.6691 18.6097 15.1291 18.6497C15.4491 18.6797 15.7091 18.8997 15.7991 19.2097C15.8791 19.5197 15.7591 19.8397 15.4991 20.0197C14.8391 20.4697 14.0091 20.8497 12.9591 21.1897L11.3791 21.7097C10.2291 22.0697 9.22907 22.2497 8.32907 22.2497ZM7.77908 6.21968L6.48907 6.63968C2.91907 7.78968 2.06907 9.46968 3.21907 13.0497L4.49907 16.9997C5.65907 20.5697 7.33908 21.4297 10.9091 20.2797L12.4891 19.7597C12.5491 19.7397 12.5991 19.7197 12.6591 19.6997L11.5991 19.4497C6.98908 18.3597 5.47907 15.9197 6.55907 11.3197L7.53907 7.12968C7.60908 6.80968 7.68907 6.49968 7.77908 6.21968Z" fill="white"/>
                                        <path d="M17.4894 10.5098C17.4294 10.5098 17.3694 10.4998 17.2994 10.4898L12.4494 9.25978C12.0494 9.15978 11.8094 8.74978 11.9094 8.34978C12.0094 7.94978 12.4194 7.70978 12.8194 7.80978L17.6694 9.03978C18.0694 9.13978 18.3094 9.54978 18.2094 9.94978C18.1294 10.2798 17.8194 10.5098 17.4894 10.5098Z" fill="white"/>
                                        <path d="M14.5592 13.8899C14.4992 13.8899 14.4392 13.8799 14.3692 13.8699L11.4592 13.1299C11.0592 13.0299 10.8192 12.6199 10.9192 12.2199C11.0192 11.8199 11.4292 11.5799 11.8292 11.6799L14.7392 12.4199C15.1392 12.5199 15.3792 12.9299 15.2792 13.3299C15.1992 13.6699 14.8992 13.8899 14.5592 13.8899Z" fill="white"/>
                                        </svg>
                                        
                                        
                                </div>
                                <div class="text-white">
                                    عدد العروض
                                    <p class="t-c">
                                        10 عروض
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex col-md-4">
                                <div class="t-c">

                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M13.4002 17.4201H10.8902C9.25016 17.4201 7.92016 16.0401 7.92016 14.3401C7.92016 13.9301 8.26016 13.5901 8.67016 13.5901C9.08016 13.5901 9.42016 13.9301 9.42016 14.3401C9.42016 15.2101 10.0802 15.9201 10.8902 15.9201H13.4002C14.0502 15.9201 14.5902 15.3401 14.5902 14.6401C14.5902 13.7701 14.2802 13.6001 13.7702 13.4201L9.74016 12.0001C8.96016 11.7301 7.91016 11.1501 7.91016 9.36008C7.91016 7.82008 9.12016 6.58008 10.6002 6.58008H13.1102C14.7502 6.58008 16.0802 7.96008 16.0802 9.66008C16.0802 10.0701 15.7402 10.4101 15.3302 10.4101C14.9202 10.4101 14.5802 10.0701 14.5802 9.66008C14.5802 8.79008 13.9202 8.08008 13.1102 8.08008H10.6002C9.95016 8.08008 9.41016 8.66008 9.41016 9.36008C9.41016 10.2301 9.72016 10.4001 10.2302 10.5801L14.2602 12.0001C15.0402 12.2701 16.0902 12.8501 16.0902 14.6401C16.0802 16.1701 14.8802 17.4201 13.4002 17.4201Z" fill="white"/>
                                        <path d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z" fill="white"/>
                                        <path d="M15 22.75H9C3.57 22.75 1.25 20.43 1.25 15V9C1.25 3.57 3.57 1.25 9 1.25H15C20.43 1.25 22.75 3.57 22.75 9V15C22.75 20.43 20.43 22.75 15 22.75ZM9 2.75C4.39 2.75 2.75 4.39 2.75 9V15C2.75 19.61 4.39 21.25 9 21.25H15C19.61 21.25 21.25 19.61 21.25 15V9C21.25 4.39 19.61 2.75 15 2.75H9Z" fill="white"/>
                                        </svg>
                                        
                                        
                                </div>
                                <div class="text-white">

                                    الايجار السنوي
                                    <p class="t-c">
                                        من 20 ألف -50 ألف
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex col-md-4">
                                <div class="t-c">

                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12.0009 14.1704C9.87086 14.1704 8.13086 12.4404 8.13086 10.3004C8.13086 8.16043 9.87086 6.44043 12.0009 6.44043C14.1309 6.44043 15.8709 8.17043 15.8709 10.3104C15.8709 12.4504 14.1309 14.1704 12.0009 14.1704ZM12.0009 7.94043C10.7009 7.94043 9.63086 9.00043 9.63086 10.3104C9.63086 11.6204 10.6909 12.6804 12.0009 12.6804C13.3109 12.6804 14.3709 11.6204 14.3709 10.3104C14.3709 9.00043 13.3009 7.94043 12.0009 7.94043Z" fill="white"/>
                                        <path d="M11.9997 22.76C10.5197 22.76 9.02969 22.2 7.86969 21.09C4.91969 18.25 1.65969 13.72 2.88969 8.33C3.99969 3.44 8.26969 1.25 11.9997 1.25C11.9997 1.25 11.9997 1.25 12.0097 1.25C15.7397 1.25 20.0097 3.44 21.1197 8.34C22.3397 13.73 19.0797 18.25 16.1297 21.09C14.9697 22.2 13.4797 22.76 11.9997 22.76ZM11.9997 2.75C9.08969 2.75 5.34969 4.3 4.35969 8.66C3.27969 13.37 6.23969 17.43 8.91969 20C10.6497 21.67 13.3597 21.67 15.0897 20C17.7597 17.43 20.7197 13.37 19.6597 8.66C18.6597 4.3 14.9097 2.75 11.9997 2.75Z" fill="white"/>
                                        </svg>
                                        
                                </div>
                                <div class="text-white">

                                    المكان
                                    <p class="t-c">
                                    {{place}}
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex col-md-4">
                                <div class="t-c">

                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12 22.75C6.76 22.75 2.5 18.49 2.5 13.25C2.5 8.01 6.76 3.75 12 3.75C17.24 3.75 21.5 8.01 21.5 13.25C21.5 18.49 17.24 22.75 12 22.75ZM12 5.25C7.59 5.25 4 8.84 4 13.25C4 17.66 7.59 21.25 12 21.25C16.41 21.25 20 17.66 20 13.25C20 8.84 16.41 5.25 12 5.25Z" fill="white"/>
                                        <path d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z" fill="white"/>
                                        <path d="M15 2.75H9C8.59 2.75 8.25 2.41 8.25 2C8.25 1.59 8.59 1.25 9 1.25H15C15.41 1.25 15.75 1.59 15.75 2C15.75 2.41 15.41 2.75 15 2.75Z" fill="white"/>
                                        </svg>
                                        
                                </div>
                                <div class="text-white">

                                    باقي عن غلق الجولة
                                    <p class="t-c">
                                        {{restDay}} يوم
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex col-md-4">
                                <div class="t-c">

                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M20.01 19.26C19.82 19.26 19.63 19.19 19.48 19.04L14.53 14.09C14.24 13.8 14.24 13.32 14.53 13.03C14.82 12.74 15.3 12.74 15.59 13.03L20.54 17.98C20.83 18.27 20.83 18.75 20.54 19.04C20.39 19.19 20.2 19.26 20.01 19.26Z" fill="white"/>
                                        <path d="M10.1095 18.4297C9.37946 18.4297 8.67946 18.1397 8.16946 17.6197L3.92945 13.3797C2.85945 12.3097 2.85945 10.5597 3.92945 9.48966L10.9995 2.41969C12.0695 1.34969 13.8195 1.34969 14.8895 2.41969L19.1295 6.65968C19.6495 7.17968 19.9395 7.86968 19.9395 8.59968C19.9395 9.32968 19.6495 10.0297 19.1295 10.5397L12.0595 17.6097C11.5395 18.1497 10.8495 18.4297 10.1095 18.4297ZM12.9395 3.11967C12.6195 3.11967 12.2995 3.23966 12.0595 3.48966L4.98947 10.5597C4.49947 11.0497 4.49947 11.8397 4.98947 12.3297L9.22946 16.5697C9.69946 17.0397 10.5195 17.0397 10.9995 16.5697L18.0695 9.49967C18.3095 9.25967 18.4395 8.94967 18.4395 8.61967C18.4395 8.28967 18.3095 7.96966 18.0695 7.73966L13.8295 3.49967C13.5795 3.23967 13.2595 3.11967 12.9395 3.11967Z" fill="white"/>
                                        <path d="M8 21.75H2C1.59 21.75 1.25 21.41 1.25 21C1.25 20.59 1.59 20.25 2 20.25H8C8.41 20.25 8.75 20.59 8.75 21C8.75 21.41 8.41 21.75 8 21.75Z" fill="white"/>
                                        <path d="M13.63 15.7403C13.44 15.7403 13.25 15.6703 13.1 15.5203L6.03 8.45035C5.74 8.16035 5.74 7.68035 6.03 7.39035C6.32 7.10035 6.8 7.10035 7.09 7.39035L14.16 14.4603C14.45 14.7503 14.45 15.2303 14.16 15.5203C14.02 15.6703 13.82 15.7403 13.63 15.7403Z" fill="white"/>
                                        </svg>
                                        
                                        
                                </div>
                                <div class="text-white">
                                    الملكية المطروحة
                                    <p class="t-c">
                                        {{ownership}} %
                                    </p>
                                </div>
                            </div>
                            <div class="d-flex col-md-4">
                                <div class="t-c">

                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12 22.7498C11.3 22.7498 10.59 22.4798 10.06 21.9498L8.34998 20.2598C7.92998 19.8398 7.35001 19.6097 6.76001 19.6097H6C3.93 19.6097 2.25 17.9398 2.25 15.8898V4.97974C2.25 2.92974 3.93 1.25977 6 1.25977H18C20.07 1.25977 21.75 2.92974 21.75 4.97974V15.8898C21.75 17.9398 20.07 19.6097 18 19.6097H17.24C16.65 19.6097 16.07 19.8498 15.65 20.2598L13.94 21.9498C13.41 22.4798 12.7 22.7498 12 22.7498ZM6 2.74976C4.76 2.74976 3.75 3.74973 3.75 4.96973V15.8798C3.75 17.1098 4.76 18.0997 6 18.0997H6.76001C7.75001 18.0997 8.70997 18.4998 9.40997 19.1898L11.12 20.8798C11.61 21.3598 12.4 21.3598 12.89 20.8798L14.6 19.1898C15.3 18.4998 16.26 18.0997 17.25 18.0997H18C19.24 18.0997 20.25 17.0998 20.25 15.8798V4.96973C20.25 3.73973 19.24 2.74976 18 2.74976H6Z" fill="white"/>
                                        <path d="M11.9999 10.7498C10.2999 10.7498 8.91992 9.3698 8.91992 7.6698C8.91992 5.9698 10.2999 4.58984 11.9999 4.58984C13.6999 4.58984 15.08 5.9698 15.08 7.6698C15.08 9.3698 13.6999 10.7498 11.9999 10.7498ZM11.9999 6.08984C11.1299 6.08984 10.4199 6.7998 10.4199 7.6698C10.4199 8.5398 11.1299 9.24982 11.9999 9.24982C12.8699 9.24982 13.58 8.5398 13.58 7.6698C13.58 6.7998 12.8699 6.08984 11.9999 6.08984Z" fill="white"/>
                                        <path d="M16 16.4103C15.59 16.4103 15.25 16.0703 15.25 15.6603C15.25 14.2803 13.79 13.1504 12 13.1504C10.21 13.1504 8.75 14.2803 8.75 15.6603C8.75 16.0703 8.41 16.4103 8 16.4103C7.59 16.4103 7.25 16.0703 7.25 15.6603C7.25 13.4503 9.38 11.6504 12 11.6504C14.62 11.6504 16.75 13.4503 16.75 15.6603C16.75 16.0703 16.41 16.4103 16 16.4103Z" fill="white"/>
                                        </svg>
                                        
                                        
                                </div>
                                <div class="text-white">

                                    اسم الناشر 
                                    <p class="t-c">
                                        {{publisher}}
                                    </p>
                                </div>
                            </div>
                        

                        </div>

                      </div>
                </div>
</template>

<script>
export default {
 name:'offer-investment-project',
 props:{
    title:{
        type:String
    },
    datePublish:{
        type:String
    },
  publisher:{
        type:String
    },
    ownership:{
        type:[String,Number]
    },
    place:{
        type:String
    },
    description:{
        type:String
    },
    restDay:{
        type:[String,Number]
    }
   
 }
}
</script>

<style>

</style>