<template>
    <div class="border-bottom mt-3" >
  <div class="d-flex justify-content-between d-flex gap-2 align-items-center ">
                            <div class="d-flex gap-2 align-items-">
                                <p :class="{open:status=='active',end:status=='expired'}">
                                    {{statusText}}
                                </p>
                                <p style="color: #003439;">
                                    {{title}}
                                </p>
                            </div>
                            <div>
                                <button v-if="status=='active'" class="btn border-danger text-danger mt-3">
                                    الغاء الاشتراك
                                </button>
                            </div>
                        </div>
                        <div class="d-flex gap-2">
                                    
                                    <p class="t-c">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M17.74 22.75H6.26C3.77 22.75 1.75 20.73 1.75 18.24V11.51C1.75 9.02001 3.77 7 6.26 7H17.74C20.23 7 22.25 9.02001 22.25 11.51V12.95C22.25 13.36 21.91 13.7 21.5 13.7H19.48C19.13 13.7 18.81 13.83 18.58 14.07L18.57 14.08C18.29 14.35 18.16 14.72 18.19 15.1C18.25 15.76 18.88 16.29 19.6 16.29H21.5C21.91 16.29 22.25 16.63 22.25 17.04V18.23C22.25 20.73 20.23 22.75 17.74 22.75ZM6.26 8.5C4.6 8.5 3.25 9.85001 3.25 11.51V18.24C3.25 19.9 4.6 21.25 6.26 21.25H17.74C19.4 21.25 20.75 19.9 20.75 18.24V17.8H19.6C18.09 17.8 16.81 16.68 16.69 15.24C16.61 14.42 16.91 13.61 17.51 13.02C18.03 12.49 18.73 12.2 19.48 12.2H20.75V11.51C20.75 9.85001 19.4 8.5 17.74 8.5H6.26Z" fill="#979797"/>
                                            <path d="M2.5 13.16C2.09 13.16 1.75 12.82 1.75 12.41V7.84006C1.75 6.35006 2.69 5.00001 4.08 4.47001L12.02 1.47001C12.84 1.16001 13.75 1.27005 14.46 1.77005C15.18 2.27005 15.6 3.08005 15.6 3.95005V7.75003C15.6 8.16003 15.26 8.50003 14.85 8.50003C14.44 8.50003 14.1 8.16003 14.1 7.75003V3.95005C14.1 3.57005 13.92 3.22003 13.6 3.00003C13.28 2.78003 12.9 2.73003 12.54 2.87003L4.6 5.87003C3.79 6.18003 3.24 6.97006 3.24 7.84006V12.41C3.25 12.83 2.91 13.16 2.5 13.16Z" fill="#979797"/>
                                            <path d="M19.6005 17.7999C18.0905 17.7999 16.8105 16.6799 16.6905 15.2399C16.6105 14.4099 16.9105 13.5999 17.5105 13.0099C18.0205 12.4899 18.7205 12.2 19.4705 12.2H21.5505C22.5405 12.23 23.3005 13.0099 23.3005 13.9699V16.03C23.3005 16.99 22.5405 17.7699 21.5805 17.7999H19.6005ZM21.5305 13.7H19.4805C19.1305 13.7 18.8105 13.8299 18.5805 14.0699C18.2905 14.3499 18.1505 14.7299 18.1905 15.1099C18.2505 15.7699 18.8805 16.2999 19.6005 16.2999H21.5605C21.6905 16.2999 21.8105 16.18 21.8105 16.03V13.9699C21.8105 13.8199 21.6905 13.71 21.5305 13.7Z" fill="#979797"/>
                                            <path d="M14 12.75H7C6.59 12.75 6.25 12.41 6.25 12C6.25 11.59 6.59 11.25 7 11.25H14C14.41 11.25 14.75 11.59 14.75 12C14.75 12.41 14.41 12.75 14 12.75Z" fill="#979797"/>
                                            </svg>
                                            {{price}} {{ $t('riyals') }}
                                    </p>
                           
                                    <p class="t-c">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill="#737373"/>
                                            <path d="M15.71 15.9298C15.58 15.9298 15.45 15.8998 15.33 15.8198L12.23 13.9698C11.46 13.5098 10.89 12.4998 10.89 11.6098V7.50977C10.89 7.09977 11.23 6.75977 11.64 6.75977C12.05 6.75977 12.39 7.09977 12.39 7.50977V11.6098C12.39 11.9698 12.69 12.4998 13 12.6798L16.1 14.5298C16.46 14.7398 16.57 15.1998 16.36 15.5598C16.21 15.7998 15.96 15.9298 15.71 15.9298Z" fill="#737373"/>
                                            </svg>
                                            
                                            تاريخ  بداية الاشتراك : {{dateStart}}
                                    </p>
                                    <p class="t-c">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill="#737373"/>
                                            <path d="M15.71 15.9298C15.58 15.9298 15.45 15.8998 15.33 15.8198L12.23 13.9698C11.46 13.5098 10.89 12.4998 10.89 11.6098V7.50977C10.89 7.09977 11.23 6.75977 11.64 6.75977C12.05 6.75977 12.39 7.09977 12.39 7.50977V11.6098C12.39 11.9698 12.69 12.4998 13 12.6798L16.1 14.5298C16.46 14.7398 16.57 15.1998 16.36 15.5598C16.21 15.7998 15.96 15.9298 15.71 15.9298Z" fill="#737373"/>
                                            </svg>
                                            
                                            ينتهي في {{dateEnd}}
                                    </p>
                            
                                   
                                </div>
                    </div>
</template>

<script>
export default {
 props:{
    status:{
        type:String // active | expired | inactive
    },
    title:{
        type:String,
    },
    price:{
        type:[Number,String]
    },
    dateStart:{
        type:String
    },
    dateEnd:{
        type:String
    }
 },
 data:()=>({

 }),
 computed:{
    statusText(){
        switch (this.status) {
            case 'active': return this.$t('status_subscribe_active')
            case 'finished': return this.$t('status_subscribe_expired')
        
            default:return ''
        }
    }
 }
}
</script>

<style scoped>
.open {
    background-color: #1FB9B3;
    color: white;
    border-radius: 5px;
    padding: 3px 13px;
    margin: 0;
    line-height: 1;
    height: 24px;
}
.end {
    background-color: #414042;
    color: white;
    border-radius: 5px;
    padding: 3px 13px;
    margin: 0;
    line-height: 1;
    height: 24px;
}
</style>