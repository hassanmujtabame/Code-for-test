<template>
  <div class="box-subscribe border shadow p-5  text-center rounded-2 bg-white d-flex flex-column justify-content-between">
    <div>
    ...
    <div>
      <a @click="selected" v-if="!subscribed || !subscribed.subscribe" href="#" class="btn-main px-4 py-2">{{
        $t('subscribe-now') }}</a>
      <a @click="selected" v-else-if="(subscribed.subscribe && subscribed.type == 'free' && subscribed.package_id !== itemId)"
        href="#" class="btn-main px-4 py-2">رقي حسابك</a>
      <a v-else-if="subscribed.package_id == itemId" href="#" class="btn-danger px-4 py-2">انت مشترك الان</a>
    </div>
  </div>
</template>
