<template>
    <div class="student-course box border" style="height: 361px; width:100%">
        <div class="student-course__image">
            <router-link custom v-slot="{ navigate }" :to="getRouteLocale('academy-course-show', { id: item.id })">

                <img @click="goToCourse(navigate, $event)" class="w-100 rounded-top" :src="item.image_path"
                    :alt="item.title" height="100%">
            </router-link>
        </div>
        <router-link style="color: black" :to="getRouteLocale('academy-course-show', { id: item.id })">
            <div class="py-3 ">
                <div class="d-flex justify-content-between align-items-cent px-3" style="align-items: baseline;">
                    ...
                    <div class="d-flex">
                        ...
                        <img class="mx-2" style="width: 20px; height: 20px;"
                ...
                <div v-if="item.user_info" class="border-top mt-2 py-2 w-100">
                    ...
                    <div class="px-3 user_info_description" v-if="item.user_info.description">{{ item.user_info.description
                    }}
                    </div>
                </div>
            </div>
