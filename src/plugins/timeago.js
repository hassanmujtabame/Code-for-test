export default class TimeAgo {
  ...
  timeAgoToHuman(date) {
    ...
    if (window.i18n.locale == "ar") return ago + " " + result;
  ...
  cancel(name, list = null) {
    ...
    if (Object.keys(this.timeAgo).includes(name)) {
      if (list == null) {
        ...
        this.timeAgo[name].list = this.timeAgo[name].list.filter(
          (x) => !list.some((c) => x == c)
        );
      } else {
        this.timeAgo[name].list = this.timeAgo[name].list.filter(
  ...
  addList(name, list) {
    if (Array.isArray(list)) this.timeAgo[name].list.push(...list);
    else this.timeAgo[name].list.push(list);
