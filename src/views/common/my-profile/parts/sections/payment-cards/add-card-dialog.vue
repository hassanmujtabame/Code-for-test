<template>
  <d-dialog-large mlg :group="group" :closeDialog="closeDialog" :openDialog="openDialog">
    ...
    <template v-slot:actions>
      <button @click="save" type="button" class="btn btn-main">{{ $t('save') }}</button>
    </template>
  </d-dialog-large>
</template>
<script>
...
export default {
  ...
  methods: {
    async save() {
      ...
      try {
        let { data } = this.card_payment_id
          ? await userAPI.updateCreditCardInfo(this.card_payment_id, formData)
          : await userAPI.addCreditCardInfo(formData);
        if (data.success) {
          let card = {
          ...
          this.$emit("success", {
            card,
            type: this.card_payment_id ? "update" : "add"
          });
          this.closeEvent();
        }
      } catch (error) {
