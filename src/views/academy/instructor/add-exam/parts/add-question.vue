    ...
    <div class="add-question-item border p-3 rounded-3 h-100 d-flex flex-column">
        <div class="add-question-item__wrapper box flex-grow-1">
                ...
                <div class="row">
                    <div class="col-12 row">
                        <div class="col-12 col-md-5">
                            <d-text-input solo type="text" v-model="opt.title" class=" border w-100 p-2 rounded-2"
                                label="اضف الاختيار">
                                <!--append-icon-->
                        ...
                        <div class="col-12 col-md-5">
                            <d-text-input solo type="text" v-model="opt.id"
                                class="col-12 col-md-6 border w-100 p-2 rounded-2 mx-2" label="قيمة الاختيار">
                            </d-text-input>
                        </div>
                    </div>
                    <!--correct-->
                    <div class="col-12 form-check mt-3">
                        <input class="form-check-input" v-model="isCorrect" type="checkbox"
                            :name="`correctRadios-${item.uuid}`" :value="1">
                        <label class="form-check-label clickable fs-r-12-17" style="color:var(--label-color)"
                            :for="`correctRadios-${item.uuid}`">
                            هذا جواب صحيح
                        </label>
                    </div>
                </div>
                <button v-if="selectedIndex === false" @click="addOption" class="btn h-100"><i
                        class="m-c fa  fa-circle-plus clickable"></i></button>
                <template v-else>
            ...
            <ValidationObserver ref="form" tag="div">
                <div class="mt-3">
                    <validationProvider :name="$t('question-title')" vid="title" rules="required" v-slot="{ errors }">
                        <d-text-input :errors="errors" v-model="itemLocal.title" class="border w-100 rounded-2"
                            label="السؤال">
                        </d-text-input>
                    </validationProvider>
                </div>
                ...
                <div v-else class="add-question-item__content mt-3">
                    <div v-for="(o, i) in itemLocal.options" :key="i" class="form-check form-check-option mt-1"
                        :class="{ 'correct-answer': o.is_correct }">
                        <input class="form-check-input" v-model="value_" type="radio" :name="`exampleRadios-${item.uuid}`"
                            :id="`exampleRadios-${o.id}`" :value="o.id">
                        <label class="form-check-label clickable" :for="`exampleRadios-${item.uuid}`">
                            {{ o.title }} <i>[{{ o.id }}]</i>
                        </label>
                        <div class="add-question-item__actions">
                            <i @click="confirmDeleteOption(o, i)" :disabled="i === selectedIndex"
                                class="fa fa-trash c-danger"></i>
                            <i @click="editOption(o, i)" :disabled="i === selectedIndex"
                                class="fa fa-pen-to-square c-info"></i>
                        </div>
                    </div>
                </div>
            </ValidationObserver>
        ...
        <div class="add-question-item__footer mt-3 text-start  flex-shrink-0">
            <button @click="save" :disabled="adding" class="btn btn-custmer px-3" role="button">
