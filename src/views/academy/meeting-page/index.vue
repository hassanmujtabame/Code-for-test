        ...
        <div class="row">
          <div class="col-md-6">
            <div>
              ...
              <div>
                <button class="btn bg-main p-2 px-4 text-white">أنضم الى اللقاء</button>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div>
              ...
              <iframe height="384" class="rounded-3 w-100 " :src="meeting.video" title="مقطع تعريفي عن شركة رياديات"
                frameborder="0"
          ...
          <d-swiper is-auto :items="items">
            <template v-slot="{ item }">
              <router-link :to="getRouteLocale('academy-learning-meeting-show', { id: item.id })">
                <meetingCard :item="item" :img="item.img" :title="item.title" :type="item.type" :date="item.date" />
              </router-link>
            </template>
          </d-swiper>
        </div>
