                    ...
                    <div v-if="showDialog" class="row">
                        ...
                        <div v-for="(item, i) in items" :key="i" class="col-3">
                            <CardVue :title="item.name" :img="item.image_path" @click-image="onChoose(item)" :size="90"
                                :class="{ 'selected': selected === item,'disabled':userSubacademys.some(x=>x.subscribe && x.department_id==item.id) }" />
                        </div>
...
import CardVue from '@/components/cards/academy-dept-circle.vue'
    ...
    methods: {
        ...
        async initializing() {
            ...
            try {
                ...
                if (data.success) {
                    this.items = data.data.map((dept) => { return { ...dept,subscribed:this.userSubacademys.some(x=>x.subscribe && x.department_id==dept.id), choosen: false } }).sort((a,b)=>a.subscribed && !b.subscribed?1:-1)
...
<style scoped>
.selected >>> .academy-dept__title{
   color:var(--color-primary) 
}
.selected >>> .academy-dept__image{
    box-shadow: 0px 4px 15px 1px rgba(255, 0, 0, 0.25);
}
.disabled >>> .academy-dept__image{
    filter: contrast(0.2);
}
</style>