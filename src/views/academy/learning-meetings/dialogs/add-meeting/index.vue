<template>
  <d-dialog-large :xl="false" :hideHeader="step == 3" :group="group" :open-dialog="openDialog" :close-dialog="closeDialog">
    ...
    <template v-slot:default>
      ...
      <div v-if="showDialog" ref="form" tag="div">
        <ValidationObserver class="form-step" ref="form1" id="form-step-1" v-show="step == 1">
          <!--meeting-type-->
          <div class="mt-3">
            <ValidationProvider :name="$t('meeting-type')" vid="type" rules="required" v-slot="{ errors }"
              v-if="step == 1">
          ...
          <div v-if="itemForm.type == 'on-site'" class="mt-3">
            <ValidationProvider :name="$t('meeting-title')" vid="title" rules="required" v-slot="{ errors }"
              v-if="step == 1">
              <d-text-input name="address_onSite" :errors="errors" v-model="itemForm.address_onSite"
                label="عنوان المقر وتفاصيله" />
            </ValidationProvider>
          </div>
          <div class="mt-3">
            <ValidationProvider :name="$t('meeting-title')" vid="title" rules="required" v-slot="{ errors }"
          ...
          <div class="mt-3" name="start_date" v-if="itemForm.type != 'recored'">
            <ValidationProvider :name="$t('meeting-date')" vid="start_date" rules="required" v-slot="{ errors }"
              v-if="step == 1">
          ...
          <div class="mt-3 d-flex justify-content-around gap-1" v-if="itemForm.type != 'recored'">
            <ValidationProvider :name="$t('start-time')" tag="div" class="flex-grow-1" vid="start_time" rules="required"
              v-slot="{ errors }" v-if="step == 1">
              <d-text-input name="start_time" :errors="errors" type="time" v-model="itemForm.start_time"
                :label="$t('start-time')" />
            </ValidationProvider>
            <ValidationProvider :name="$t('end-time')" tag="div" class="flex-grow-1" vid="end_time" rules="required"
              v-slot="{ errors }" v-if="step == 1">
              <d-text-input name="end_time" :errors="errors" type="time" v-model="itemForm.end_time"
                :label="$t('end-time')" />
            </ValidationProvider>
          </div>
          <!--specialite -->
          <div class="mt-3" name="category_id">
            <ValidationProvider :name="$t('meeting-field')" vid="category_id" rules="required" v-slot="{ errors }"
              v-if="step == 1">
              <d-multiselect-input :errors="errors" label="تخصص اللقاء" :opts="categories" track-id="id" label-name="name"
                v-model="itemForm.category_id" seperate=" - " />
            </ValidationProvider>
          </div>
          <div class="mt-3" v-if="itemForm.type == 'live'">
            <ValidationProvider :name="$t('meeting-url')" vid="meeting_url"
              :rules="itemForm.type == 'live' ? 'required' : ''" v-slot="{ errors }" v-if="step == 1">
              <d-text-input name="meeting_url" type="text" :errors="errors" v-model="itemForm.meeting_url"
                :label="$t('meeting-url')" />
            </ValidationProvider>
          </div>

          <div class="mt-3" v-if="itemForm.type == 'on-site'">
            <div class="my-2" style="border: 1px solid">
              رابط العنوان على خرائط جوجل
            </div>

            <google-map @address_map="getAddressMap" />
          </div>
          <div class="mt-3">
            <ValidationProvider :name="$t('about-meeting')" vid="content" rules="required" v-slot="{ errors }"
              v-if="step == 1">
              <d-textarea-input name="content" type="text" :errors="errors" rows="10" v-model="itemForm.content"
                label="عن اللقاء" />
            </ValidationProvider>
          </div>
        </ValidationObserver>
        ...
        <ValidationObserver class="form-step" ref="form2" id="form-step-2" v-show="step == 2">
          <ValidationProvider :name="$t('Image')" vid="image" :rules="imageRules" v-slot="{ validate, errors }">
            <div class="m-auto">
              <div class="col-md-12 text-center">
                ...
                <input name="image" @change="uploadImage($event, validate) || validate($event)"
        ...
        <ValidationObserver ref="form-video" v-slot="{ invalid }" class="form-step" v-show="step == 3">
          ...
          <ValidationProvider v-if="itemForm.type == 'recored' && step == 3" tag="div" :name="$t('lecture-video')"
            ...
            <div class="add-lecture-video position-relative">
              ...
              <label for="video-add-lecture" :style="{ 'pointer-events': videoing ? 'none' : 'auto' }" class="form-label file-label first w-100">
                <div class="text-center p-5">
...
export default {
  ...
  methods: {
        ...
        if (data.success) {
          ...
          let dataEvt = {
            ...
            btns: [
              {
                title: "عرض اللقاء",
                action: () => this.$router.replace({name: 'academy-learning-meeting-show', params: { id: this.itemForm.meeting_id }}),
              },
            ]
          };
          // if (this.itemForm.id) {
    ...
    openDialog(dataEvt) {
      ...
      this.$nextTick(() => {
        this.makeVideoEmpty(), this.makeImageEmpty();
      });
      return true;
    },
    closeDialog() {
