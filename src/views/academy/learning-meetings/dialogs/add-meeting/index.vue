<template>
  ...
  >
          ...
          <div v-if="itemForm.type == 'on-site'" class="mt-3">
            ...
            >
              <d-text-input
                name="address_onSite"
                :errors="errors"
                v-model="itemForm.address_onSite"
            ...
            >
              <div class="m-auto">
                <div class="col-md-12 text-center">
                  <label
                    for="imginput"
                    class="img-zone form-label file-label first w-100"
                  >
          ...
          >
            ...
            >
              <div class="add-lecture-video position-relative">
                ...
                <label
                  for="video-add-lecture"
                  :style="{ 'pointer-events': videoing ? 'none' : 'auto' }"
                ...
                >
                  ...
                  <input
                    v-show="itemForm.meeting_id"
                    @change="uploadVideo($event, validate) || validate($event)"
              ...
              <div class="w-100 text-center mt-2">
                <button
                  @click="saveVideo"
                  :disabled="invalid || videoing || !itemForm.video"
                  class="btn btn-custmer"
                >
                  <i v-if="!videoing" class="fa fa-upload"></i>
                  <i v-else> {{ percentVideo }} % </i>
                  {{ $t("upload-video") }}
                </button>
                <button
                  v-if="itemForm.video && false"
                  @click="emptyVideo()"
                  class="btn btn-custmer bg-danger"
                >
                  {{ $t("clear-video") }}
                </button>
              </div>
    ...
    <template v-slot:actions>
      ...
      <button
        style="color: #2cb7b3; border: none; background-color: transparent"
        v-if="step != 3"
        @click="save"
        :disabled="loading"
  ...
  methods: {
    ...
    async saveVideo() {
      ...
      try {
        ...
        let { data } = await academyAPI.meetingsAPI.addVideo(
          this.itemForm.meeting_id,
          formData,
          config
        );
        if (data.success) {
