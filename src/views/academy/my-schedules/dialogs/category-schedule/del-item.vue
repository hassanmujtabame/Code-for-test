...
<svg width="160" height="160" viewBox="0 0 160 160" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 <p class="mt-1">
  {{ $t('confirm_delete_category_schedule_message') }}
 </p>
 <p class="m-c m-0">
    {{blog.title}}
 </p>
...
export default {
 props:{
    group:{
        type:String,
        default:'delete-category-schedule'
    }
 },
 data:()=>({
    blog:{id:null},
 ...
 methods:{
    async delBlog(){
        try {
            let {data} = await categoriesScheduleAPI.deleteItem(this.blog.id)
            if(data.success){
                this.closeEvent()
                this.router_push('service-provider-my-schedules')
