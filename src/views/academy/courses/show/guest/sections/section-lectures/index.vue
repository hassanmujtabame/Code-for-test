    ...
    <div>
      ...
      <d-expanded-panel :id="('accordionFlushExample')"  >
        <d-expanded-panel-item :title="$t('lectures')" :opened='true' >
          <div v-for="(lecture, c) in lectures" :key="('c' + c)" class="course-lectures__item">
            <div class="course-lectures__item-tile">{{ lecture.title }}</div>
            <div v-if="lecture.is_free === true || c === 0" class="course-lectures__item-action">
              <a href="#" @click="watchVideo(lecture, $event)" class="m-c"><i class="fa fa-circle-play"></i> شاهد مجانا</a>
            </div>
          </div>
...
export default {
  ...
  methods: {
    watchVideo(lecture, evt) {
      ...
      this.fireOpenDialog('watch-lecture-course-dialog', { page: this.itemPage, item: lecture })
    },
    async initializing() {
      this.loading = true;
      try {
