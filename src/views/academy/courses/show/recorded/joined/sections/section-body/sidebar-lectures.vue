<template>
  <div
    ref="courseOffcanvas"
    class="course-show-page__lectures"
    :class="{ 'offcanvas offcanvas-end': isTablet || isMobile }"
    tabindex="-1"
    :id="drawerId"
    aria-labelledby="course-menu-drawerLabel"
  >
    <div
      class="course-show-page__lectures-close"
      :class="{ show: isTablet || isMobile }"
    >
      <button
        :id="drawerId + '-close'"
    ...
    <div class="course-show-page__lectures-content">
      ...
      <div class="course-show-page__lectures-content-wrapper">
        ...
        >
          <transition-group>
            <li
              @click="selected(lect, i)"
              class="course-show-page__lecture"
              :class="{
                selected: lect.uuid === selectedLecture,
                'lecture-project': ['project', 'projects'].includes(lect.type),
              }"
              v-for="(lect, i) in lectures"
              :key="i"
            >
              <span class="course-show-page__title"
                >{{ i + 1 }}. {{ lect.title }}</span
              >
              <span v-if="canEdit" class="course-show-page__actions">
                ...
                <button
                  v-if="!isDraggable"
                  class="btn"
                  @click="showConfirmDeleteItem(lect)"
                >
              ...
              <span
                ><i
                  v-if="lect.is_completed"
                  style="color: green"
                  class="fa fa-circle-check px-1"
                ></i
              ></span>
              <span
                v-if="['lecture'].includes(lect.type) || !lect.type"
                class="course-show-page__time"
                >{{ lect.time }}</span
              >
            </li>
          </transition-group>
        ...
        <div v-if="course.exams.length > 0">
          ...
          >
            <li
              @click="selected(exam, i)"
              class="course-show-page__lecture lecture-project"
              :class="{ selected: exam.uuid === selectedLecture }"
              v-for="(exam, i) in course.exams"
              :key="i"
            >
              <span class="course-show-page__title"
                >{{ i + 1 }}. {{ exam.title }}</span
              ...
              <span v-if="canEdit" class="course-show-page__actions">
                ...
                <template v-else>
                  <button
                    class="btn"
                    @click="showEditDialog({ ...exam, type: 'exam' })"
                  >
                    <i class="fa fa-pen" style="color: blue"></i>
                  </button>
                  <button
                    class="btn"
                    @click="showConfirmDeleteItem({ ...exam, type: 'exam' })"
                  >
                    <i class="fa-solid fa-trash" style="color: red"></i>
                  </button>
                </template>
...
export default {
  ...
  methods: {
    ...
    showAddDialog(type) {
      ...
      console.log("ts", type);
      this.fireOpenDialog(`add-${type}-course-dialog`, {
        page: this.itemPage,
    ...
    showEditDialog(lect) {
      ...
      if (lect.type == "exam") dialog = "add-exam-course-dialog";
      if (dialog)
    ...
    async deleteItem(item) {
      if (item.type == "exam") await this.deleteExam(item);
      else await this.deleteLecture(item);
    },
    ...
    updateLectures({ item, type }) {
      ...
      if (type == "update") {
        if (item.type == "exam") {
          ...
          if (index > -1) {
            this.course.exams[index] = Object.assign(
              this.course.exams[index],
              item
            );
          }
        } else {
      ...
      if (type == "add") {
        if (item.type == "exam") this.course.exams.push(item);
        else this.lectures.push(item);
      }
      if (type == "all") {
        this.initializing();
      }
    ...
    async onEnd(evt) {
      ...
      this.loading = true;
      try {
        await academyAPI.lecturesAPI.orderItem(ob.id, {
          level: evt.newIndex + 1,
        });
      } catch (error) {
        //
      }
    ...
    selected(lect) {
      ...
      if (
        this.userAcademyRole == "student" &&
        !this.itemPage.user_is_join_course
      ) {
        let DataEvt = {
