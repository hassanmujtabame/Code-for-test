  ...
  <div ref="courseOffcanvas" class="course-show-page__lectures"
    ...
    <div class="course-show-page__lectures-content">
      ...
      <div class="course-show-page__lectures-content-wrapper">
        <draggable :move="onMove" @end="onEnd" tag="ol" group="lectures" v-model="lectures" ghost-class="ghost"
          handle=".handle">

          <transition-group>
            <li @click="selected(lect, i)" class="course-show-page__lecture"
              :class="{ 'selected': lect.uuid === selectedLecture, 'lecture-project': ['project', 'projects'].includes(lect.type) }"
              v-for="(lect, i) in lectures" :key="i">
              ...
              <span v-if="canEdit" class="course-show-page__actions">
                ...
                <button v-if="!isDraggable" class="btn " @click="showEditDialog(lect)"><i class="fa fa-pen"
                    style="color:blue"></i></button>
                <button v-if="!isDraggable" class="btn " @click="showConfirmDeleteItem(lect)"><i class="fa-solid fa-trash"
                    style="color:red"></i></button>
              </span>
              <span><i v-if="lect.is_completed" style="color:green" class="fa fa-circle-check px-1"></i></span>
              <span v-if="['lecture'].includes(lect.type) || !lect.type" class="course-show-page__time">{{ lect.time
              }}</span>
            </li>
          </transition-group>
        </draggable>
        <div v-if="course.exams.length > 0">
          ...
          <draggable @end="onEndExam" tag="ol" group="exams" v-model="lectures" ghost-class="ghost" handle=".handle">
            <li @click="selected(exam, i)" class="course-show-page__lecture lecture-project"
              :class="{ 'selected': exam.uuid === selectedLecture }" v-for="(exam, i) in course.exams" :key="i">
...
export default {
  ...
  data: (vm) => {
    return {
      course: {
        ...vm.itemPage, exams: vm.itemPage.exams.map(l => {
