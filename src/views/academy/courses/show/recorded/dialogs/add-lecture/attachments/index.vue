<template>
    <d-course-panel :class-header="`d-flex justify-content-between ${isMobile ? 'flex-column' : ''}`">
        <template v-slot:header>
            <h1 class="flex-grown-1">{{ $t('attachments') }} : </h1>
            <ValidationObserver ref="form" class="flex-shrink-0" tag="div" v-slot="{ invalid }">
                <div class="d-flex align-items-center">
                    <ValidationProvider tag="div" :name="$t('attachment-title')" vid="title" rules="required"
                        v-slot="{ errors }" class="">
                        <d-text-input :disabled="!lectureId" :errors="errors" v-model="itemForm.title"
                            label="اسم المرفق الجديد">
                            <!--after error-->
                            <template v-slot:after-error>
                                <div class="d-flex" style="font-size: 10px;" v-if="itemForm.file"><span
                                        class="flex-grow-1">{{ itemForm.file.name }} </span><span class="flex-shrink-0"
                                        style="color:red">{{ percentage }} %</span></div>
                            </template>
                            <!--prend-icon-->
                            <template v-slot:prend-icon>
                                <ValidationProvider tag="div" :name="$t('attachment')" vid="file" rules="required" class=""
                                    v-slot="{ errors, validate }">
                                    <label class="btn position-relative">
                                        <i class="fa fa-paperclip" :class="[lectureId ? 'm-c' : 't-c']"></i>
                                        <input type="file" :disabled="!lectureId"
                                            @change="uploadFile($event, validate) || validate($event)"
                                            class="position-absolute top-0 left-0 w-100 h-100" style="opacity: 0;">
                                    </label>
                            ...
                            <template v-slot:append-icon>
                                <div class="">
                                    <button @click="uploadAttachment" :disabled="!lectureId || invalid || loading"
                                        class="btn no-border" :class="{ 'c-save': !invalid, 't-c': invalid }">
        ...
        <template v-slot:default>
            <div>
                <rateCard v-for="(rateItem, i) in attachments" :key="i" :item="rateItem" @delete="showConfirmDeleteItem"
    ...
    methods: {
        ...
        async deleteItem(item) {
            ...
            try {
                ...
                let { data } = await academyAPI.lecturesAPI.deleteAttachment(item.id)
                if (data.success) {
        ...
        async uploadAttachment() {
            ...
            try {
                ...
                let { data } = await academyAPI.lecturesAPI.addAttachment(this.lectureId, formData, config)
                if (data.success) {
