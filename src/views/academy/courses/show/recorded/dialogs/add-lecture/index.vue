      ...
      <div v-if="showDialog" class="row">
                  ...
                  <template v-slot:append-icon>
                    <div class="">
                      <button
                        ...
                        class="btn no-border"
                        :class="{ 'c-save': !invalid, 't-c': invalid }"
                      >
        ...
        <div class="col-md-6" v-if="lectureId">
          <ValidationObserver tag="div" ref="form-video" v-slot="{ invalid }">
            ...
            >
              <div
                class="add-lecture-video position-relative"
                :class="{ 'video-disabled': !lectureId }"
              >
                ...
                <label
                  ...
                  :class="{ 'label-disabled': !lectureId }"
                >
              ...
              <div class="w-100 text-center mt-2">
                <button
                  @click="saveVideo"
                  :disabled="
                    !lectureId || invalid || videoing || !itemForm.video
                  "
...
export default {
  ...
  computed: {
    titleDialog() {
      return this.itemForm.id
        ? `${this.$t("lecture-modification")}: <span class="m-c">${
  ...
  methods: {
    async saveTitle() {
      ...
      try {
        let { data } = this.itemForm.id
          ? await academyAPI.lecturesAPI.updateItem(this.itemForm.id, formData)
          : await academyAPI.lecturesAPI.addItem(this.itemPage.id, formData);
        if (data.success) {
          if (this.itemForm.id) {
    ...
    async saveVideo() {
      ...
      try {
        let { data } = await academyAPI.lecturesAPI.addVideo(
          this.lectureId,
          formData,
          config
        );
        if (data.success) {
          ...
          if (this.itemForm.id) {
            ...
            this.eventUpdateLectures(
              { video: this.itemForm.video, type: "lecture" },
              "update"
            );
            window.SwalSuccess("تم اضافه الفيديو بنجاح");
            this.closeDialog();
          } else {
            this.itemForm.id = data.data.lesson_id;
            this.$emit("add", { ...this.itemForm });
