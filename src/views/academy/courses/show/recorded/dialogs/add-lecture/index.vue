<template>
  <d-dialog-large :group="group" :loading="loading" :closeDialog="closeDialog" :openDialog="openDialog" :xl="!!lectureId">
      ...
      <div v-if="showDialog" class="row">
        <div :class="[lectureId ? 'col-md-6' : 'col-12']">
          <div>
            <ValidationObserver tag="div" ref="form-title" v-slot="{ invalid }">
              <ValidationProvider tag="div" :name="$t('lecture-title')" vid="title" rules="required" v-slot="{ errors }">
                <d-text-input type="text" v-model="itemForm.title" :errors="errors" label="عنوان الدرس ">
                  <!--append-icon-->
                  <template v-slot:append-icon>
                    <div class="">
                      <button @click="saveTitle" :disabled="invalid || saving" class="btn no-border"
        ...
        <div class="col-md-6" v-if="lectureId">
          <ValidationObserver tag="div" ref="form-video" v-slot="{ invalid }">
            <ValidationProvider tag="div" :name="$t('lecture-video')" vid="video" rules="required|ext:mp4"
              ...
              <div class="add-lecture-video position-relative" :class="{ 'video-disabled': !lectureId }">
                ...
                <label for="imginput" class="form-label file-label first w-100" :class="{ 'label-disabled': !lectureId }">
                  ...
                  <input v-show="lectureId" @change="uploadVideo($event, validate) || validate($event)"
              ...
              <div class="w-100 text-center mt-2">
                <button @click="saveVideo" :disabled="!lectureId || invalid || videoing || !itemForm.video"
