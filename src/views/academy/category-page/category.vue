<template>
  <div class="mt-5 p-3 overflow-x-hidden">
    <div class="container">
      <div class="d-flex justify-content-center align-items-center container">
        <!-- <h1>تعرفي على مجالات التسويق لدينا</h1> -->
        <h1 style="font-size: 42px">
          أستكشف مجالات <span style="color: #1fb9b3"> رياديات </span>
        </h1>
      </div>
      <d-swiper :slides-per-view="5" is-auto :space-between="10" :items="items">
        <template v-slot:default="{ item }">
          <CardVue :title="item.title" :img="item.img" :icon="item.icon" />
        </template>
      </d-swiper>
    </div>
  </div>
...
<script>
import CardVue from "./card-vue-academy.vue";
export default {
  name: "section-recognize-our-marketing",
  components: {
    CardVue,
  },
  data: (vm) => {
