        ...
        <div class="container mt-5">
            <d-filter-list :call-list="loadList" hideSide classColCard="col-12 mt-2" @change="changeFilter">
                <template v-slot:total>
                    <h4 class="fw-bold">طلباتك لاستثمار مع الاخرين</h4>
                </template>
                ...
                <template v-slot:default="{ item }">
                    ...
                    <InvestProjectRequest :itemId="item.id" :status="item.status" :title="item.title"
                        :userName="item.user_info.name" :dateRequest="item.created_at" :place="item.city"
    ...
    data: () => {
        return {
            ...
            actions: [
                ...
                { status: 'approve', label: 'طلبات تم اختيارك بها' },
                { status: 'waiting', label: 'لم تتم الموافقة بعد' },
                { status: 'disapprove', label: 'تم استبعادك' }
            ],
            filterItem: {
                search: null,
                created_at: 'asc',
