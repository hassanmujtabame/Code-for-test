...
<svg width="160" height="160" viewBox="0 0 160 160" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 <p class="mt-1">
  {{ $t('confirm_delete_podcast_message') }}
 </p>
 <p class="m-c m-0">
    {{itemForm.title}}
 </p>
    ...
    <template v-slot:actions>
        <button @click="delItem" style="height: 40px;" class="btn btn-main"> {{$t('confirm_delete')}}</button>
    </template>
  </d-dialog-large>
</template>
...
export default {
 ...
 methods:{
    async delItem(){
        try {
            let {data} = await podcastsAPI.deleteItem(this.itemForm.id)
            if(data.success){
                console.log('success',data)
                this.closeEvent()
                this.$router.push(this.getRouteLocale('network-podcasts'))
            }
        } catch (error) {
