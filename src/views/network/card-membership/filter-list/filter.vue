<template>
    <div class="mt-5">
                <ul class="nav nav-pills mb-3 rounded-2 align-items-center gap-2" id="pills-tab" role="tablist">
                  
                
                    <li v-for="(btn, i) in categories" :key="i" class="nav-item" >
                <button @click="changeCategories(btn.id)" class="btn"  :class="{ active: category_id === btn.id }" type="button">{{ btn.name }}</button>
            </li>
                </ul>
...
background-color: rgba(var(--color-nav-grey)/.2);
border-radius: 4px;
}
