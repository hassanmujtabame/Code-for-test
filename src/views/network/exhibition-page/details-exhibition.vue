<template>
  <div class="box border rounded-3 p-2 mt-3">
    <div class="d-flex flex-column w-100 gap-2">
      <img class="shadow" :src="itemPage.image" :alt="itemPage.title" />
      <button
        v-if="isOwner"
        @click="router_push('network-exhibition-requests', { id: itemPage.id })"
        class="btn px-5 py-2 text-light"
        style="background-color: #1fb9b3"
      >
        قائمة طلبات المشاركة في المعرض
      </button>
      <button @click="openDemandParticipateDialog" v-else class="btn-main">
        شارك في المعرض
      </button>
    </div>
    <h3 class="border-bottom py-2">تفاصيل المعرض</h3>
    <p style="color: #888" class="mt-3" v-html="itemPage.content"></p>
    <div class="">
      <div class="box" v-html="itemPage.details"></div>
      <div class="pt-3">
        <h3 class="border-bottom py-2">تفاصيل الخدمة</h3>
        <div class="row mt-3">
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M12.0019 16.3299C9.61187 16.3299 7.67188 14.3899 7.67188 11.9999C7.67188 9.60992 9.61187 7.66992 12.0019 7.66992C14.3919 7.66992 16.3319 9.60992 16.3319 11.9999C16.3319 14.3899 14.3919 16.3299 12.0019 16.3299ZM12.0019 9.16992C10.4419 9.16992 9.17188 10.4399 9.17188 11.9999C9.17188 13.5599 10.4419 14.8299 12.0019 14.8299C13.5619 14.8299 14.8319 13.5599 14.8319 11.9999C14.8319 10.4399 13.5619 9.16992 12.0019 9.16992Z"
                fill="#979797"
              />
              <path
                d="M11.9981 21.0205C8.23812 21.0205 4.68813 18.8205 2.24812 15.0005C1.18813 13.3505 1.18813 10.6605 2.24812 9.00047C4.69812 5.18047 8.24813 2.98047 11.9981 2.98047C15.7481 2.98047 19.2981 5.18047 21.7381 9.00047C22.7981 10.6505 22.7981 13.3405 21.7381 15.0005C19.2981 18.8205 15.7481 21.0205 11.9981 21.0205ZM11.9981 4.48047C8.76813 4.48047 5.67813 6.42047 3.51813 9.81047C2.76813 10.9805 2.76813 13.0205 3.51813 14.1905C5.67813 17.5805 8.76813 19.5205 11.9981 19.5205C15.2281 19.5205 18.3181 17.5805 20.4781 14.1905C21.2281 13.0205 21.2281 10.9805 20.4781 9.81047C18.3181 6.42047 15.2281 4.48047 11.9981 4.48047Z"
                fill="#979797"
              />
            </svg>
            <p>
              المشاهدات :
              <span class="m-c">
                {{ itemPage.views ?? "N/A" }}
              </span>
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="25"
              height="25"
              viewBox="0 0 25 25"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M17.7439 22.752H6.26391C3.77391 22.752 1.75391 20.7319 1.75391 18.2419V11.512C1.75391 9.02196 3.77391 7.00195 6.26391 7.00195H17.7439C20.2339 7.00195 22.2539 9.02196 22.2539 11.512V12.952C22.2539 13.362 21.9139 13.702 21.5039 13.702H19.4839C19.1339 13.702 18.8139 13.832 18.5839 14.072L18.5739 14.082C18.2939 14.352 18.1639 14.7219 18.1939 15.1019C18.2539 15.7619 18.8839 16.2919 19.6039 16.2919H21.5039C21.9139 16.2919 22.2539 16.6319 22.2539 17.0419V18.2319C22.2539 20.7319 20.2339 22.752 17.7439 22.752ZM6.26391 8.50195C4.60391 8.50195 3.25391 9.85196 3.25391 11.512V18.2419C3.25391 19.9019 4.60391 21.252 6.26391 21.252H17.7439C19.4039 21.252 20.7539 19.9019 20.7539 18.2419V17.8019H19.6039C18.0939 17.8019 16.8139 16.6819 16.6939 15.2419C16.6139 14.4219 16.9139 13.612 17.5139 13.022C18.0339 12.492 18.7339 12.202 19.4839 12.202H20.7539V11.512C20.7539 9.85196 19.4039 8.50195 17.7439 8.50195H6.26391Z"
                fill="#979797"
              />
              <path
                d="M2.50391 13.162C2.09391 13.162 1.75391 12.822 1.75391 12.412V7.84201C1.75391 6.35201 2.69391 5.00196 4.08391 4.47196L12.0239 1.47196C12.8439 1.16196 13.7539 1.27201 14.4639 1.77201C15.1839 2.27201 15.6039 3.082 15.6039 3.952V7.75199C15.6039 8.16199 15.2639 8.50199 14.8539 8.50199C14.4439 8.50199 14.1039 8.16199 14.1039 7.75199V3.952C14.1039 3.572 13.9239 3.22199 13.6039 3.00199C13.2839 2.78199 12.9039 2.73198 12.5439 2.87198L4.60391 5.87198C3.79391 6.18198 3.24391 6.97201 3.24391 7.84201V12.412C3.25391 12.832 2.91391 13.162 2.50391 13.162Z"
                fill="#979797"
              />
              <path
                d="M19.6063 17.8031C18.0963 17.8031 16.8163 16.6831 16.6963 15.2431C16.6163 14.4131 16.9163 13.6031 17.5163 13.0131C18.0263 12.4931 18.7263 12.2031 19.4763 12.2031H21.5563C22.5463 12.2331 23.3063 13.0131 23.3063 13.9731V16.0331C23.3063 16.9931 22.5463 17.7731 21.5863 17.8031H19.6063ZM21.5363 13.7031H19.4863C19.1363 13.7031 18.8163 13.8331 18.5863 14.0731C18.2963 14.3531 18.1563 14.7331 18.1963 15.1131C18.2563 15.7731 18.8863 16.3031 19.6063 16.3031H21.5663C21.6963 16.3031 21.8163 16.1831 21.8163 16.0331V13.9731C21.8163 13.8231 21.6963 13.7131 21.5363 13.7031Z"
                fill="#979797"
              />
              <path
                d="M14.0039 12.752H7.00391C6.59391 12.752 6.25391 12.412 6.25391 12.002C6.25391 11.592 6.59391 11.252 7.00391 11.252H14.0039C14.4139 11.252 14.7539 11.592 14.7539 12.002C14.7539 12.412 14.4139 12.752 14.0039 12.752Z"
                fill="#979797"
              />
            </svg>

            <p>
              سعر الدخول :
              <span class="m-c">
                {{
                  itemPage.price === 0
                    ? $t("free-enter")
                    : `${itemPage.price} ${$t("riyals")}`
                }}
              </span>
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.92 19.7496C10.63 19.7496 10.35 19.7096 10.12 19.6196C6.7 18.4496 1.25 14.2696 1.25 8.0896C1.25 4.9196 3.80999 2.34961 6.95999 2.34961C8.46999 2.34961 9.87999 2.92959 10.93 3.95959C11.99 2.92959 13.39 2.34961 14.9 2.34961C18.05 2.34961 20.61 4.9196 20.61 8.0896C20.61 8.5196 20.58 8.95961 20.52 9.40961C20.49 9.63961 20.35 9.8496 20.15 9.9696C19.95 10.0896 19.7 10.0996 19.48 10.0096C18.25 9.47961 16.72 9.87962 15.92 10.9496C15.78 11.1396 15.51 11.2396 15.32 11.2496C15.08 11.2496 14.86 11.1396 14.72 10.9496C14.15 10.1896 13.28 9.75961 12.34 9.75961C10.71 9.75961 9.38 11.0996 9.38 12.7496C9.38 15.3496 10.87 17.1996 12.13 18.3096C12.31 18.4696 12.4 18.6996 12.38 18.9396C12.36 19.1796 12.23 19.3896 12.02 19.5196C11.91 19.5796 11.82 19.6196 11.77 19.6296C11.51 19.7096 11.22 19.7496 10.92 19.7496ZM6.95999 3.84961C4.63999 3.84961 2.75 5.7496 2.75 8.0896C2.75 13.3196 7.10001 16.5396 9.60001 17.7796C8.65001 16.5096 7.87 14.8296 7.87 12.7396C7.87 10.2596 9.86999 8.2496 12.33 8.2496C13.45 8.2496 14.5 8.65962 15.31 9.38962C16.32 8.48962 17.76 8.07959 19.1 8.32959C19.1 8.24959 19.1 8.16961 19.1 8.09961C19.1 5.75961 17.21 3.85959 14.89 3.85959C13.56 3.85959 12.33 4.47959 11.52 5.54959C11.38 5.73959 11.16 5.84961 10.92 5.84961C10.68 5.84961 10.46 5.73959 10.32 5.54959C9.53001 4.46959 8.29999 3.84961 6.95999 3.84961Z"
                fill="#979797"
              />
              <path
                d="M15.3206 21.65C15.0906 21.65 14.8506 21.62 14.6606 21.55C13.8006 21.26 12.4106 20.55 11.1406 19.42C9.66062 18.11 7.89062 15.89 7.89062 12.74C7.89062 10.26 9.89062 8.25 12.3506 8.25C13.4706 8.25 14.5206 8.66001 15.3306 9.39001C16.5906 8.26001 18.5106 7.94 20.0906 8.63C21.7106 9.35 22.7606 10.97 22.7606 12.75C22.7606 17.49 18.6006 20.67 15.9806 21.56C15.7806 21.62 15.5506 21.65 15.3206 21.65ZM12.3406 9.75C10.7106 9.75 9.38062 11.09 9.38062 12.74C9.38062 15.34 10.8706 17.19 12.1306 18.3C13.2406 19.28 14.4106 19.89 15.1406 20.13C15.2206 20.16 15.4106 20.16 15.4806 20.13C16.9806 19.62 21.2506 17.17 21.2506 12.74C21.2506 11.55 20.5506 10.47 19.4806 10C18.2706 9.47 16.7206 9.87 15.9206 10.94C15.7806 11.13 15.5106 11.23 15.3206 11.24C15.0806 11.24 14.8606 11.13 14.7206 10.94C14.1506 10.18 13.2806 9.75 12.3406 9.75Z"
                fill="#979797"
              />
            </svg>
            <p>
              المشاركين :
              <span class="m-c"> {{ itemPage.subscribers ?? "N/A" }} </span>
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M11.9989 14.1695C9.86891 14.1695 8.12891 12.4395 8.12891 10.2995C8.12891 8.15945 9.86891 6.43945 11.9989 6.43945C14.1289 6.43945 15.8689 8.16945 15.8689 10.3095C15.8689 12.4495 14.1289 14.1695 11.9989 14.1695ZM11.9989 7.93945C10.6989 7.93945 9.62891 8.99945 9.62891 10.3095C9.62891 11.6195 10.6889 12.6795 11.9989 12.6795C13.3089 12.6795 14.3689 11.6195 14.3689 10.3095C14.3689 8.99945 13.2989 7.93945 11.9989 7.93945Z"
                fill="#979797"
              />
              <path
                d="M11.9977 22.76C10.5177 22.76 9.02773 22.2 7.86773 21.09C4.91773 18.25 1.65773 13.72 2.88773 8.33C3.99773 3.44 8.26774 1.25 11.9977 1.25C11.9977 1.25 11.9977 1.25 12.0077 1.25C15.7377 1.25 20.0077 3.44 21.1177 8.34C22.3377 13.73 19.0777 18.25 16.1277 21.09C14.9677 22.2 13.4777 22.76 11.9977 22.76ZM11.9977 2.75C9.08774 2.75 5.34773 4.3 4.35773 8.66C3.27773 13.37 6.23773 17.43 8.91773 20C10.6477 21.67 13.3577 21.67 15.0877 20C17.7577 17.43 20.7177 13.37 19.6577 8.66C18.6577 4.3 14.9077 2.75 11.9977 2.75Z"
                fill="#979797"
              />
            </svg>
            <p>
              المدينة :
              <span class="m-c"> {{ itemPage.city }} </span>
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M17.2195 9.75H8.95953C8.39953 9.75 7.83954 9.56001 7.39954 9.20001L5.20953 7.45001C4.60953 6.97001 4.26953 6.26 4.26953 5.5C4.26953 4.74 4.60953 4.01999 5.20953 3.54999L7.39954 1.79999C7.83954 1.44999 8.39953 1.25 8.95953 1.25H17.2195C18.5995 1.25 19.7195 2.37 19.7195 3.75V7.25C19.7195 8.63 18.5995 9.75 17.2195 9.75ZM8.95953 2.75C8.72953 2.75 8.50954 2.83 8.33954 2.97L6.14954 4.72C5.90954 4.91 5.76953 5.2 5.76953 5.5C5.76953 5.8 5.90954 6.09 6.14954 6.28L8.33954 8.03C8.51954 8.17 8.73953 8.25 8.95953 8.25H17.2195C17.7695 8.25 18.2195 7.8 18.2195 7.25V3.75C18.2195 3.2 17.7695 2.75 17.2195 2.75H8.95953Z"
                fill="#979797"
              />
              <path
                d="M15.0608 19.75H6.80078C5.42078 19.75 4.30078 18.63 4.30078 17.25V13.75C4.30078 12.37 5.42078 11.25 6.80078 11.25H15.0608C15.6308 11.25 16.1808 11.44 16.6208 11.8L18.8108 13.55C19.4108 14.03 19.7508 14.74 19.7508 15.5C19.7508 16.26 19.4108 16.98 18.8108 17.45L16.6208 19.2C16.1808 19.56 15.6308 19.75 15.0608 19.75ZM6.80078 12.75C6.25078 12.75 5.80078 13.2 5.80078 13.75V17.25C5.80078 17.8 6.25078 18.25 6.80078 18.25H15.0608C15.2908 18.25 15.5108 18.17 15.6808 18.03L17.8708 16.28C18.1108 16.09 18.2508 15.8 18.2508 15.5C18.2508 15.2 18.1108 14.91 17.8708 14.72L15.6808 12.97C15.5008 12.83 15.2808 12.75 15.0608 12.75H6.80078Z"
                fill="#979797"
              />
              <path
                d="M12 12.75C11.59 12.75 11.25 12.41 11.25 12V9C11.25 8.59 11.59 8.25 12 8.25C12.41 8.25 12.75 8.59 12.75 9V12C12.75 12.41 12.41 12.75 12 12.75Z"
                fill="#979797"
              />
              <path
                d="M12 22.75C11.59 22.75 11.25 22.41 11.25 22V19C11.25 18.59 11.59 18.25 12 18.25C12.41 18.25 12.75 18.59 12.75 19V22C12.75 22.41 12.41 22.75 12 22.75Z"
                fill="#979797"
              />
              <path
                d="M15 22.75H9C8.59 22.75 8.25 22.41 8.25 22C8.25 21.59 8.59 21.25 9 21.25H15C15.41 21.25 15.75 21.59 15.75 22C15.75 22.41 15.41 22.75 15 22.75Z"
                fill="#979797"
              />
            </svg>
            <p>
              تصنيف المعرض :
              <span class="m-c"> {{ itemPage.categoryName ?? "N/A" }} </span>
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M12 20.25C7.45 20.25 3.75 16.55 3.75 12C3.75 7.45 7.45 3.75 12 3.75C16.55 3.75 20.25 7.45 20.25 12C20.25 16.55 16.55 20.25 12 20.25ZM12 5.25C8.28 5.25 5.25 8.28 5.25 12C5.25 15.72 8.28 18.75 12 18.75C15.72 18.75 18.75 15.72 18.75 12C18.75 8.28 15.72 5.25 12 5.25Z"
                fill="#979797"
              />
              <path
                d="M12 15.75C9.93 15.75 8.25 14.07 8.25 12C8.25 9.93 9.93 8.25 12 8.25C14.07 8.25 15.75 9.93 15.75 12C15.75 14.07 14.07 15.75 12 15.75ZM12 9.75C10.76 9.75 9.75 10.76 9.75 12C9.75 13.24 10.76 14.25 12 14.25C13.24 14.25 14.25 13.24 14.25 12C14.25 10.76 13.24 9.75 12 9.75Z"
                fill="#979797"
              />
              <path
                d="M12 4.75C11.59 4.75 11.25 4.41 11.25 4V2C11.25 1.59 11.59 1.25 12 1.25C12.41 1.25 12.75 1.59 12.75 2V4C12.75 4.41 12.41 4.75 12 4.75Z"
                fill="#979797"
              />
              <path
                d="M4 12.75H2C1.59 12.75 1.25 12.41 1.25 12C1.25 11.59 1.59 11.25 2 11.25H4C4.41 11.25 4.75 11.59 4.75 12C4.75 12.41 4.41 12.75 4 12.75Z"
                fill="#979797"
              />
              <path
                d="M12 22.75C11.59 22.75 11.25 22.41 11.25 22V20C11.25 19.59 11.59 19.25 12 19.25C12.41 19.25 12.75 19.59 12.75 20V22C12.75 22.41 12.41 22.75 12 22.75Z"
                fill="#979797"
              />
              <path
                d="M22 12.75H20C19.59 12.75 19.25 12.41 19.25 12C19.25 11.59 19.59 11.25 20 11.25H22C22.41 11.25 22.75 11.59 22.75 12C22.75 12.41 22.41 12.75 22 12.75Z"
                fill="#979797"
              />
            </svg>
            <p>
              العنوان :
              <span class="m-c">
                <a target="_blank" :href="itemPage.address">أضغط هنا </a></span
              >
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M8 5.75C7.59 5.75 7.25 5.41 7.25 5V2C7.25 1.59 7.59 1.25 8 1.25C8.41 1.25 8.75 1.59 8.75 2V5C8.75 5.41 8.41 5.75 8 5.75Z"
                fill="#979797"
              />
              <path
                d="M16 5.75C15.59 5.75 15.25 5.41 15.25 5V2C15.25 1.59 15.59 1.25 16 1.25C16.41 1.25 16.75 1.59 16.75 2V5C16.75 5.41 16.41 5.75 16 5.75Z"
                fill="#979797"
              />
              <path
                d="M8.5 14.5023C8.37 14.5023 8.24 14.4723 8.12 14.4223C7.99 14.3723 7.89 14.3023 7.79 14.2123C7.61 14.0223 7.5 13.7723 7.5 13.5023C7.5 13.3723 7.53 13.2423 7.58 13.1223C7.63 13.0023 7.7 12.8923 7.79 12.7923C7.89 12.7023 7.99 12.6323 8.12 12.5823C8.48 12.4323 8.93 12.5123 9.21 12.7923C9.39 12.9823 9.5 13.2423 9.5 13.5023C9.5 13.5623 9.49 13.6323 9.48 13.7023C9.47 13.7623 9.45 13.8223 9.42 13.8823C9.4 13.9423 9.37 14.0023 9.33 14.0623C9.3 14.1123 9.25 14.1623 9.21 14.2123C9.02 14.3923 8.76 14.5023 8.5 14.5023Z"
                fill="#979797"
              />
              <path
                d="M12 14.4989C11.87 14.4989 11.74 14.4689 11.62 14.4189C11.49 14.3689 11.39 14.2989 11.29 14.2089C11.11 14.0189 11 13.7689 11 13.4989C11 13.3689 11.03 13.2389 11.08 13.1189C11.13 12.9989 11.2 12.8889 11.29 12.7889C11.39 12.6989 11.49 12.6289 11.62 12.5789C11.98 12.4189 12.43 12.5089 12.71 12.7889C12.89 12.9789 13 13.2389 13 13.4989C13 13.5589 12.99 13.6289 12.98 13.6989C12.97 13.7589 12.95 13.8189 12.92 13.8789C12.9 13.9389 12.87 13.9989 12.83 14.0589C12.8 14.1089 12.75 14.1589 12.71 14.2089C12.52 14.3889 12.26 14.4989 12 14.4989Z"
                fill="#979797"
              />
              <path
                d="M15.5 14.4989C15.37 14.4989 15.24 14.4689 15.12 14.4189C14.99 14.3689 14.89 14.2989 14.79 14.2089C14.75 14.1589 14.71 14.1089 14.67 14.0589C14.63 13.9989 14.6 13.9389 14.58 13.8789C14.55 13.8189 14.53 13.7589 14.52 13.6989C14.51 13.6289 14.5 13.5589 14.5 13.4989C14.5 13.2389 14.61 12.9789 14.79 12.7889C14.89 12.6989 14.99 12.6289 15.12 12.5789C15.49 12.4189 15.93 12.5089 16.21 12.7889C16.39 12.9789 16.5 13.2389 16.5 13.4989C16.5 13.5589 16.49 13.6289 16.48 13.6989C16.47 13.7589 16.45 13.8189 16.42 13.8789C16.4 13.9389 16.37 13.9989 16.33 14.0589C16.3 14.1089 16.25 14.1589 16.21 14.2089C16.02 14.3889 15.76 14.4989 15.5 14.4989Z"
                fill="#979797"
              />
              <path
                d="M8.5 17.9992C8.37 17.9992 8.24 17.9692 8.12 17.9192C8 17.8692 7.89 17.7992 7.79 17.7092C7.61 17.5192 7.5 17.2592 7.5 16.9992C7.5 16.8692 7.53 16.7392 7.58 16.6192C7.63 16.4892 7.7 16.3792 7.79 16.2892C8.16 15.9192 8.84 15.9192 9.21 16.2892C9.39 16.4792 9.5 16.7392 9.5 16.9992C9.5 17.2592 9.39 17.5192 9.21 17.7092C9.02 17.8892 8.76 17.9992 8.5 17.9992Z"
                fill="#979797"
              />
              <path
                d="M12 17.9992C11.74 17.9992 11.48 17.8892 11.29 17.7092C11.11 17.5192 11 17.2592 11 16.9992C11 16.8692 11.03 16.7392 11.08 16.6192C11.13 16.4892 11.2 16.3792 11.29 16.2892C11.66 15.9192 12.34 15.9192 12.71 16.2892C12.8 16.3792 12.87 16.4892 12.92 16.6192C12.97 16.7392 13 16.8692 13 16.9992C13 17.2592 12.89 17.5192 12.71 17.7092C12.52 17.8892 12.26 17.9992 12 17.9992Z"
                fill="#979797"
              />
              <path
                d="M15.5 18.0009C15.24 18.0009 14.98 17.8909 14.79 17.7109C14.7 17.6209 14.63 17.5109 14.58 17.3809C14.53 17.2609 14.5 17.1309 14.5 17.0009C14.5 16.8709 14.53 16.7409 14.58 16.6209C14.63 16.4909 14.7 16.3809 14.79 16.2909C15.02 16.0609 15.37 15.9509 15.69 16.0209C15.76 16.0309 15.82 16.0509 15.88 16.0809C15.94 16.1009 16 16.1309 16.06 16.1709C16.11 16.2009 16.16 16.2509 16.21 16.2909C16.39 16.4809 16.5 16.7409 16.5 17.0009C16.5 17.2609 16.39 17.5209 16.21 17.7109C16.02 17.8909 15.76 18.0009 15.5 18.0009Z"
                fill="#979797"
              />
              <path
                d="M20.5 9.83984H3.5C3.09 9.83984 2.75 9.49984 2.75 9.08984C2.75 8.67984 3.09 8.33984 3.5 8.33984H20.5C20.91 8.33984 21.25 8.67984 21.25 9.08984C21.25 9.49984 20.91 9.83984 20.5 9.83984Z"
                fill="#979797"
              />
              <path
                d="M16 22.75H8C4.35 22.75 2.25 20.65 2.25 17V8.5C2.25 4.85 4.35 2.75 8 2.75H16C19.65 2.75 21.75 4.85 21.75 8.5V17C21.75 20.65 19.65 22.75 16 22.75ZM8 4.25C5.14 4.25 3.75 5.64 3.75 8.5V17C3.75 19.86 5.14 21.25 8 21.25H16C18.86 21.25 20.25 19.86 20.25 17V8.5C20.25 5.64 18.86 4.25 16 4.25H8Z"
                fill="#979797"
              />
            </svg>
            <p>
              مـن
              <span class="m-c"> {{ itemPage.start_date }} </span>
            </p>
          </div>
          <div class="col-md-6 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M8 5.75C7.59 5.75 7.25 5.41 7.25 5V2C7.25 1.59 7.59 1.25 8 1.25C8.41 1.25 8.75 1.59 8.75 2V5C8.75 5.41 8.41 5.75 8 5.75Z"
                fill="#979797"
              />
              <path
                d="M16 5.75C15.59 5.75 15.25 5.41 15.25 5V2C15.25 1.59 15.59 1.25 16 1.25C16.41 1.25 16.75 1.59 16.75 2V5C16.75 5.41 16.41 5.75 16 5.75Z"
                fill="#979797"
              />
              <path
                d="M8.5 14.5023C8.37 14.5023 8.24 14.4723 8.12 14.4223C7.99 14.3723 7.89 14.3023 7.79 14.2123C7.61 14.0223 7.5 13.7723 7.5 13.5023C7.5 13.3723 7.53 13.2423 7.58 13.1223C7.63 13.0023 7.7 12.8923 7.79 12.7923C7.89 12.7023 7.99 12.6323 8.12 12.5823C8.48 12.4323 8.93 12.5123 9.21 12.7923C9.39 12.9823 9.5 13.2423 9.5 13.5023C9.5 13.5623 9.49 13.6323 9.48 13.7023C9.47 13.7623 9.45 13.8223 9.42 13.8823C9.4 13.9423 9.37 14.0023 9.33 14.0623C9.3 14.1123 9.25 14.1623 9.21 14.2123C9.02 14.3923 8.76 14.5023 8.5 14.5023Z"
                fill="#979797"
              />
              <path
                d="M12 14.4989C11.87 14.4989 11.74 14.4689 11.62 14.4189C11.49 14.3689 11.39 14.2989 11.29 14.2089C11.11 14.0189 11 13.7689 11 13.4989C11 13.3689 11.03 13.2389 11.08 13.1189C11.13 12.9989 11.2 12.8889 11.29 12.7889C11.39 12.6989 11.49 12.6289 11.62 12.5789C11.98 12.4189 12.43 12.5089 12.71 12.7889C12.89 12.9789 13 13.2389 13 13.4989C13 13.5589 12.99 13.6289 12.98 13.6989C12.97 13.7589 12.95 13.8189 12.92 13.8789C12.9 13.9389 12.87 13.9989 12.83 14.0589C12.8 14.1089 12.75 14.1589 12.71 14.2089C12.52 14.3889 12.26 14.4989 12 14.4989Z"
                fill="#979797"
              />
              <path
                d="M15.5 14.4989C15.37 14.4989 15.24 14.4689 15.12 14.4189C14.99 14.3689 14.89 14.2989 14.79 14.2089C14.75 14.1589 14.71 14.1089 14.67 14.0589C14.63 13.9989 14.6 13.9389 14.58 13.8789C14.55 13.8189 14.53 13.7589 14.52 13.6989C14.51 13.6289 14.5 13.5589 14.5 13.4989C14.5 13.2389 14.61 12.9789 14.79 12.7889C14.89 12.6989 14.99 12.6289 15.12 12.5789C15.49 12.4189 15.93 12.5089 16.21 12.7889C16.39 12.9789 16.5 13.2389 16.5 13.4989C16.5 13.5589 16.49 13.6289 16.48 13.6989C16.47 13.7589 16.45 13.8189 16.42 13.8789C16.4 13.9389 16.37 13.9989 16.33 14.0589C16.3 14.1089 16.25 14.1589 16.21 14.2089C16.02 14.3889 15.76 14.4989 15.5 14.4989Z"
                fill="#979797"
              />
              <path
                d="M8.5 17.9992C8.37 17.9992 8.24 17.9692 8.12 17.9192C8 17.8692 7.89 17.7992 7.79 17.7092C7.61 17.5192 7.5 17.2592 7.5 16.9992C7.5 16.8692 7.53 16.7392 7.58 16.6192C7.63 16.4892 7.7 16.3792 7.79 16.2892C8.16 15.9192 8.84 15.9192 9.21 16.2892C9.39 16.4792 9.5 16.7392 9.5 16.9992C9.5 17.2592 9.39 17.5192 9.21 17.7092C9.02 17.8892 8.76 17.9992 8.5 17.9992Z"
                fill="#979797"
              />
              <path
                d="M12 17.9992C11.74 17.9992 11.48 17.8892 11.29 17.7092C11.11 17.5192 11 17.2592 11 16.9992C11 16.8692 11.03 16.7392 11.08 16.6192C11.13 16.4892 11.2 16.3792 11.29 16.2892C11.66 15.9192 12.34 15.9192 12.71 16.2892C12.8 16.3792 12.87 16.4892 12.92 16.6192C12.97 16.7392 13 16.8692 13 16.9992C13 17.2592 12.89 17.5192 12.71 17.7092C12.52 17.8892 12.26 17.9992 12 17.9992Z"
                fill="#979797"
              />
              <path
                d="M15.5 18.0009C15.24 18.0009 14.98 17.8909 14.79 17.7109C14.7 17.6209 14.63 17.5109 14.58 17.3809C14.53 17.2609 14.5 17.1309 14.5 17.0009C14.5 16.8709 14.53 16.7409 14.58 16.6209C14.63 16.4909 14.7 16.3809 14.79 16.2909C15.02 16.0609 15.37 15.9509 15.69 16.0209C15.76 16.0309 15.82 16.0509 15.88 16.0809C15.94 16.1009 16 16.1309 16.06 16.1709C16.11 16.2009 16.16 16.2509 16.21 16.2909C16.39 16.4809 16.5 16.7409 16.5 17.0009C16.5 17.2609 16.39 17.5209 16.21 17.7109C16.02 17.8909 15.76 18.0009 15.5 18.0009Z"
                fill="#979797"
              />
              <path
                d="M20.5 9.83984H3.5C3.09 9.83984 2.75 9.49984 2.75 9.08984C2.75 8.67984 3.09 8.33984 3.5 8.33984H20.5C20.91 8.33984 21.25 8.67984 21.25 9.08984C21.25 9.49984 20.91 9.83984 20.5 9.83984Z"
                fill="#979797"
              />
              <path
                d="M16 22.75H8C4.35 22.75 2.25 20.65 2.25 17V8.5C2.25 4.85 4.35 2.75 8 2.75H16C19.65 2.75 21.75 4.85 21.75 8.5V17C21.75 20.65 19.65 22.75 16 22.75ZM8 4.25C5.14 4.25 3.75 5.64 3.75 8.5V17C3.75 19.86 5.14 21.25 8 21.25H16C18.86 21.25 20.25 19.86 20.25 17V8.5C20.25 5.64 18.86 4.25 16 4.25H8Z"
                fill="#979797"
              />
            </svg>

            <p>
              الى
              <span class="m-c"> {{ itemPage.end_date }} </span>
            </p>
          </div>
          <!--time-->
          <div class="col-md-12 d-flex">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <rect width="24" height="24" fill="white" />
              <path
                d="M12 22.75C6.76 22.75 2.5 18.49 2.5 13.25C2.5 8.01 6.76 3.75 12 3.75C17.24 3.75 21.5 8.01 21.5 13.25C21.5 18.49 17.24 22.75 12 22.75ZM12 5.25C7.59 5.25 4 8.84 4 13.25C4 17.66 7.59 21.25 12 21.25C16.41 21.25 20 17.66 20 13.25C20 8.84 16.41 5.25 12 5.25Z"
                fill="#979797"
              />
              <path
                d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z"
                fill="#979797"
              />
              <path
                d="M15 2.75H9C8.59 2.75 8.25 2.41 8.25 2C8.25 1.59 8.59 1.25 9 1.25H15C15.41 1.25 15.75 1.59 15.75 2C15.75 2.41 15.41 2.75 15 2.75Z"
                fill="#979797"
              />
            </svg>
            <p>
              {{ convertTime(itemPage.start_time) ?? "N/A" }} حتى
              {{ convertTime(itemPage.end_time) ?? "N/A" }}
            </p>
          </div>
        </div>
        <div class="border-bottom">
          <!-- <h1>
            {{ $t("publish-date") }} :
            <span class="m-c">
              {{ itemPage.date_publish }}
            </span>
          </h1> -->
          <h5 style="color: #888">شارك المعرض</h5>
        </div>
        <div
          class="d-flex gap-4 justify-content-center p-4 icon-social-exibition"
        >
          <!--facebook-->
          <!-- <a href="https://twitter.com/share?ref_src=twsrc%5Etfw" class="twitter-share-button" data-size="large"  data-show-count="false">Tweet</a> -->

          <img
            class="h-100"
            :src="`${publicPath}assets/img/Twitter.png`"
            alt=""
          />
          <img
            class="h-100"
            :src="`${publicPath}assets/img/Instagram.png`"
            alt=""
          />
          <img
            class="h-100"
            :src="`${publicPath}assets/img/Linkedin.png`"
            alt=""
          />
          <img
            class="h-100"
            :src="`${publicPath}assets/img/Facebook.png`"
            alt=""
          />

          <!-- <span v-if="false"> <i  class="fab fa-instagram" ></i></span> -->

          <!-- <span v-if="false"> <i  class="fab fa-linkedin-in" ></i></span> -->
          <!-- <div
            class="fb-share-button"
            :data-href="currentUrl"
            data-layout="button_count"
          ></div> -->
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  props: ["itemPage", "isOwner"],
  methods: {
    convertTime(time) {
      if (!time) return time;
      return time.substring(0, 5);
    },
    openDemandParticipateDialog() {
      let dataEvt = {
        title: "",
        description: `هل لديك منتجات تخص هذا المعرض وتود حجز بوث لك يمكنك من خلاله عرض منتجاتك ؟ فقط كل ما عليك فعله هو ملئ استمارة المشاركة وسنقوم بأرسالها الى صاحب المعرض ليختار المشاركين المناسبين له , نأمل انت تكون منهم بكل تأكيد !`,
        image: `${this.publicPath}assets/img/Group 1171275567.png`,
        btns: [
          {
            title: this.$t("fill-form"),
            action: this.openDemandParticipatePage,
          },
        ],
      };
      this.showConfirmMsg(dataEvt);
      //this.fireOpenDialog('share-exhibition',this.itemPage)
    },
  },
};
</script>

<style></style>
