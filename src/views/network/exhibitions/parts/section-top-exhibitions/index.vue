            ...
            <d-swiper v-if="!loading" :slides-per-view="4" :space-between="20" is-auto :items="items">
                <template v-slot:default="{ item }">
                    <router-link class="router-link h-100" :to="getRouteLocale('network-exhibition-show', { id: item.id })">
                        <exhibitionCard class="h-100" :img="item.image"
                            :userName="item.user_info ? item.user_info.name : 'N/A'" :place="item.city" :price="item.price"
                            :title="item.title" :isShared="item.is_share" :description="item.content"
