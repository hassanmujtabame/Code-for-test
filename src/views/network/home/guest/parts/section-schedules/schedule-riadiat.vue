                    ...
                    <div class="row">
                        <div v-for="(item, i) in items.filter((x, i) => i < 3)" :key="i" class="col-6  col-lg-3">
                            <router-link class="router-link"
                                ...
                                <scheduleItem :image="item.image" :title="item.title" :time="item.time" :date="item.date" />
                            </router-link>
                        </div>
                    </div>
                                ...
                                <template #day-popover="{ day, format, masks }">
                                    <div class="text-xs text-gray-300 font-semibold text-center">
                                        ...
                                        <div :class="{ 'text-right': $i18n.locale == 'ar', 'text-left': $i18n.locale !== 'ar' }">
                                            ...
                                            <p class=" pb-1 mb-1">{{ $t('titled') }} : {{ day.attributes[0].customData.title
                                            }}</p>
                                            <p class=" pb-1 mb-1">{{ $t('at-time') }} : {{
                                                timeFormatAMPM(day.attributes[0].customData.time) }}</p>
                                        </div>
                                    </div>
                                </template>
                            </d-calendar>
