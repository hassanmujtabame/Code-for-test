  ...
  <div class="mt-5 p-3" v-if="this.items.length">
    ...
    <div class="container">
      <div class="d-flex justify-content-between align-items-center">
        ...
        <div>
          <router-link class="more" :to="getRouteLocale('network-podcasts')">{{ $t('more') }}
          </router-link>
        </div>
      </div>
      <d-swiper v-if="!loading" :slides-per-view="5" is-auto :space-between="2" :items="items">
        <template v-slot:default="{ item }">
          <podcastCard :image="item.image" :name="item.title" :description="item.time_duration" />
        </template>
      </d-swiper>
    </div>
  </div>
