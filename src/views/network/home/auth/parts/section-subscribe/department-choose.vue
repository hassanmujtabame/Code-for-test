                    ...
                    <div v-if="showDialog" class="row">
                        ...
                        <div v-for="(item, i) in items" :key="i" class="col-3">
                            <CardVue :title="item.name" :img="item.image_path" @click-image="onChoose(item)" :size="90"
                                :class="{ 'selected': selected === item,'disabled':userSubnetworks.some(x=>x.subscribe && x.department_id==item.id) }" />
                        </div>
...
import CardVue from '@/components/cards/network-dept-circle.vue'
...
export default {
    ...
    methods: {
        ...
        async initializing() {
            ...
            this.loading = true;
            try {
                let { data } = await networkAPI.getDepartments();
                if (data.success) {
                    this.items = data.data.map((dept) => { return { ...dept,subscribed:this.userSubnetworks.some(x=>x.subscribe && x.department_id==dept.id), choosen: false } }).sort((a,b)=>a.subscribed && !b.subscribed?1:-1)
                    if (process.env.NODE_ENV == 'development' && this.items.length == 1) {
                        this.items[0].image_path = 'https://test1.riadiat.sa/uploads/network/departments/1682664230.jpg';
                        this.items.push({ id: 2, name: 'مجال', image_path: 'https://test1.riadiat.sa/uploads/network/departments/1682664386.jpg', subscribed: false })
                    }
                }
            } catch (error) {
...
<style scoped>
.selected >>> .network-dept__title{
   color:var(--color-primary) 
}
.selected >>> .network-dept__image{
    box-shadow: 0px 4px 15px 1px rgba(255, 0, 0, 0.25);
}
.disabled >>> .network-dept__image{
    filter: contrast(0.2);
}
</style>