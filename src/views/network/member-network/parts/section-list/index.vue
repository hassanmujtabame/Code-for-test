<template>
  <div class="container mt-5 reverse-total-filter">
    <d-filter-list :call-list="loadList" :pluralName="$t('members')" :singleName="$t('member')"
      :searchPlaceholder="$t('search-by-name-or-field')" classColCard="col-md-3 mt-3" classSideBar="w-100" newFilter
      hideTop hideOrder :orderOpts="[
          { id: 'asc', name: 'الاحدث' },
          { id: 'desc', name: 'الاقدم', }
        ]" @change="changeFilter">
      ...
      <template v-slot="{ item }">
        <ItemCard :to="getRouteLocale('network-show-profile', { id: item.id })" :name="item.name"
          :description="item.job_title" :img="item.image" :showJob="true" />
      </template>
