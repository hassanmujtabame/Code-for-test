<template>
  <div
    style="height: 150px"
    class="consult-details pt-3 col-md-6 col-12 overflow-y-auto"
  >
    <h4 class="consultant-title">الدورات التدريبة</h4>
...
export default {
  ...
  methods: {
    async initializing() {
      this.loading = true;
      try {
        let { data } = await consultingAPI.consultants.getCourses(
          this.itemPage.id
        );
        if (data.success) {
