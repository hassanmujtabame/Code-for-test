...
<img src="/assets/img/feeling-sorry.png" />
 ...
 <p class="dialog-desc m-0">
  أخبرنا من فضلك عن سبب انزعاجك من هذه خدمة وسوف نقوم باتخاذ الإجراء المناسب
 </p>
 ...
 <div class="mt-4">
  ...
  v-model="comment" 
  rows="3" 
  placeholder="سبب الاعتراض على الخدمة"
  ></textarea>
 ...
 methods:{
    async sendAbuse(){
      ...
      if (!valid) {
        ...
        try {
            let {data} = await userAPI.sendAbuseReadyService({comment:this.comment,service_id:this.itemDialog.id})
            if(data.success){
