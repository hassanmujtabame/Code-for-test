<template>
  <d-filter-list hideSide hideTitle :call-list="loadList" @change="changeFilter"
    ...
    <template v-slot="{ item }">
        <showProposal :title="item.title" :description="item.description" :department="item.category_name"
          :category="item.field_name" :price="item.price" :execution-time="item.execution_period" :during="item.rest_days"
          :place="item.city" :offers="item.count_offer" :datePublish="item.created_at" :image="item.image"
          :state="item.state" :id="item.id" :user_info="item.user_info" :shareLink='shareLink + "/" + item.id' />
    </template>
    <!-- </d-swiper> -->
