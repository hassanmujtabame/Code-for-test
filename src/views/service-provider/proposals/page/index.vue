<template>
  <div :id="`service-${serviceId}`" style="margin-top:85px">
    ...
    <template v-else>
      <progressPage
      v-if="(itemPage.is_offer_sent && itemPage.user_offer 
      && ['underway', 'finished'].includes(itemPage.user_offer.status)) 
      || (isOwner && ['underway', 'finished'].includes(itemPage.status))"
