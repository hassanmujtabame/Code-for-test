<template>
    <div style="margin-top: 96px;">
        <div class="container">
            <div class="row justify-content-between align-items-center ">
                ...
                <div class="col-md-6 text-end">
                    <button style="height: 40px;" class="bg-main text-white border-0 rounded-2 px-3"
        ...
        <div class="d-flex flex-column h-100 z-10 overflow-hidden " :class="{'bg-day':attributes&&attributes.some(x=>x.customData) }" >
          <span class="day-label text-sm text-gray-900">{{ day.day }}</span>
          <div class="d-flex flex-column justify-content-center align-items-start flex-grow-1 overflow-y-auto overflow-x-auto">
            <div
            ...
            >
            <span :class="attr.customData.class">
             <bdi> {{ timeFormatAMPM(attr.customData.start_time,true) }} : {{ timeFormatAMPM(attr.customData.end_time,true) }}</bdi>
            </span>
             <div class="text-center w-100">
  <button class="btn" type="button" @click="showMenu(`#dropdownMenuButton-${attr.customData.id}`)">
...
export default {
 ...
 methods:{
    ...
    async loadList(){
        this.loading = true;
        try {
         let { data } = await schedulesAPI.getAll() 
         if(data.success){
            this.items = data.data
            if(process.env.NODE_ENV !== 'production')
            this.items.push(
                {id: -1, title: "تجريب", categoryName: "ويبنار تججريب",
date:new Date(),image:null,start_time:"02:37",end_time:'15:00',
class: 'text-white',
