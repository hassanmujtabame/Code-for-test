    ...
    <div v-else>
      ...
      >
        ...
        <template v-slot:search>
          <router-link
            :to="getRouteLocale('service-provider-proposal-page',{id:$route.params.id})"
            class="btn btn-custmer"
          >صفحة الطلب</router-link>
        </template>
...
export default {
  ...
  data: () => {
    return {
      ...
      status: null,
      actions: [
        { status: null, label: "كل العروض علي هذا الطلب" },
  ...
  methods: {
    ...
    async loadList(metaInfo) {
      try {
        ...
        return await myRequestsClientAPI.getOffersAll(
