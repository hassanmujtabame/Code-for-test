<template>
    <div class="my-5 blog border rounded-3">
        <d-filter-list style="background: unset !important" :call-list="loadList" @change="changeFilter"
            :searchPlaceholder="$t('search_by_service')" orderName="price" :orderOpts="[
                    { id: 'asc', name: 'الأقل سعرا' },
                    { id: 'desc', name: 'الأغلى سعرا', }
                ]" classColCard="col-md-4 mt-3" classSideBar="w-100" hideTop newFilter>
            ...
            <template v-slot="{ item }">
                <readyServiceCard :image="item.image" :title="item.title" :price="item.price" :place="item.city"
                    :state="item.state" :name="item.user_info" :description="item.desc"
                    :categoryName="item.category_name ?? 'N/A'" :status="item.status" :id="item.id" />
            </template>
            <!-- filterItem:{
            search:null,
            price:'asc',
