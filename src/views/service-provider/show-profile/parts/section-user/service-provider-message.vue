<template>
    <d-dialog-large :xl="false" :group="group" :loading="loading" :closeDialog="closeDialog" :openDialog="openDialog">
        ...
        <div>
            <ValidationObserver ref="form">
                <ValidationProvider :name="$t('title_message')" vid="title_message" rules="required" v-slot="{ errors }">
                    <input type="text" v-model="itemForm.title" class="border rounded-2 w-100  p-3 " name=""
                        :placeholder="$t('title_message')">
                    <d-error-input :errors="errors" v-if="errors.length" />
                </ValidationProvider>

                <div class="my-3">
                    <ValidationProvider :name="$t('message')" vid="message" rules="required" v-slot="{ errors }">
                        <d-textarea-input :errors="errors" v-model="itemForm.message" rows="6" class="form-control"
                            placeholder="اكتب رسالة الى صاحب الخدمة">
                        </d-textarea-input>
                    </ValidationProvider>
                </div>
                <div class="my-3">
                    <ValidationProvider :name="$t('record-audio')" vid="audio" v-slot="{ errors }">
                        <d-audio-input v-model="itemForm.audio" :errors="errors" :label="$t('add-audio-message')" />
                    </ValidationProvider>
                </div>


                <button style="display: flex; margin:auto" @click="sendMessage" class="btn btn-custmer mt-5">أرسل
                    رسالة</button>
