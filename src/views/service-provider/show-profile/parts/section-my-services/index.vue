      ...
      <div class="row p-2 order">
        <div v-for="(item, k) in items" :key="k" class="col-md-4 mt-2">
          <router-link
            class="router-link"
            :to="
              getRouteLocale('service-provider-ready-service', { id: item.id })
            "
          >
            <MyServiceCard
              :image="item.image"
    ...
    <div class="row">
      <div class="col-12">
        <button
          ...
          :class="{ 'btn-more-loading-disabled': !isThereMore }"
          class="btn-more-loading"
        >
          <plusRectRoundIcon /> {{ $t("more_services") }}
        </button>
        <div
          v-else
          class="btn-more-loading btn-more-loading-disabled text-center w-100"
        >
          لا يوجد المزيد
        </div>
      </div>
...
export default {
  ...
  computed: {
    isThereMore() {
      return (
        this.metaInfo.total_page &&
        this.metaInfo.current_page < this.metaInfo.total_page
      );
    },
  },
  methods: {
