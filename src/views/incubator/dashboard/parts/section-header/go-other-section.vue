...
<svg width="160" height="160" viewBox="0 0 160 160" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 <p class="mt-1">
الأن انت على وشك الانتقال الى حسابك في <span class="m-c">
    {{itemDialog.title}}
 </span>
 </p>
    ...
    <template v-slot:actions>
        <button @click="goToRoute" style="height: 40px;" class="btn btn-main"> {{$t('confirm_redirect')}}</button>
    </template>
  </d-dialog-large>
</template>
...
export default {
 props:{
    group:{
        type:String,
        default:'go-to-pther-section'
    }
 },
 data:()=>({
    itemDialog:{title:null,route:null},
    showed:false,
 }),
 methods:{
  goToRoute(){
    this.$router.push(this.itemDialog.route)
  },
    openDialog(data){
