    ...
    <div v-if="phase.department_name">
      ...
      <div class="student-course box border" style="width: 260px">
        <div class="student-course__image">
          ...
          <router-link
            class="router-link"
            :to="getRouteLocale('academy-course-show', { id: phase.id })"
          >
            <img
              class="w-100 rounded-top"
              :src="phase.image_path"
              height="100%"
            />
            <!-- </router-link> -->
          </router-link>
        ...
        <div class="p-3">
          ...
          <p class="student-course__description text-two-lines">
            {{ phase.desc }}
          </p>
          <div class="row">
...
export default {
  ...
  methods: {
    ...
    async downloadingModel() {
      try {
        let { data } = await modelsAPI.download(this.phase.id);
        if (data.success) {
          //download file
