<template>
  <div>
    <BodyFormCheckout :title="title" changeable @payment="payment">
  ...
  methods: {
    dataSuccessMessage() {
      return {
        ...
        description: '',
        btns: [
          { title: 'معاملاتك المالية', action: () => this.router_push('network-my-financial-transactions'), class: 'btn btn-custmer' },
          { title: 'الرئيسية', action: () => this.router_push('network-home'), class: 'btn btn-custmer-w' },
    ...
    async payment(evt) {
      ...
      try {
        let { data } = await networkAPI.rechargeBalance(pay_info)
        if (data.success) {
